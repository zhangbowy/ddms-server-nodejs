...
export class TeRedis extends EventEmitter {
    ...
    public HMGet(key: string, ids, cb) {
    ...
    public getSortedSet(key: string, maxNum: number, bDesc = true): ReSortedSet {
    ...
    public del_hash_key(key: string, subkey: string | string[]) {
...
export class ReSortedSet extends ReBase {
    ...
    */
    ...
    /**
         ...
         */
    public loadByScore(min: string | number, max: string | number, callback: Function) {
        if (this._bDesc) {
            if (this._maxNum > 0) {
...
export class ReHash extends ReBase implements ifReHash {
    ...
    * callback: function(bsuccess:boolean,self)
    */
    public load(callback:(succ:boolean,db:ReHash)=>void) {
