...
class AppUnit {
    ...
    update() {
        if (this.option.wx_token)
            this.wx_access_token_update();
    }
    wx_access_token_update() {
        if (TeConfig_1.configInst.get("access_token_update")) {
            return;
        }
        if (this.wx_access_token_timeout >= Date.now())
            return;
        // 假设有两分钟的预判，提前去获取
        /**
        appid	游戏应用id	String
         ...
         */
        let sec = Date.now();
        let sign = crypto_1.createHash('md5').update('' + this.appid + sec + this.appkey).digest('hex');
        TeHttpQuest_1.http_quest('get', `https://wxpush.shandw.com/getaccesstoken/wxdc1767838b33cbdb?appid=${this.appid}&sec=${sec}&Key=${this.appkey}&sign=${sign}`, null, (data) => {
            if (data) {
                try {
