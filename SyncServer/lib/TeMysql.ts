...
export class TeMysql extends EventEmitter {
    ...
    private _tables_func: Map<{ sql: string, comm: string, dbcomm: string }> = new Map<{ sql: string, comm: string, dbcomm: string }>();
    ...
    public queryEx(sql: string, args: any[], cb: (error: Mysql.QueryError, result: Array<Mysql.RowDataPacket>) => void) {
        this._mysqlCon.query(Mysql.format(sql), args, cb);
    }    
}
