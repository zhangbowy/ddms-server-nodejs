    ...
    */
    ...
    __onAccept__(socket) {
        ...
        socket.on('data', ((_socket, _data) => {
            // TeLog.info('----------------' + this.constructor.name + ' ' + _socket.linkid + ' : ' + _data.cmd);
            // TeLog.info('recive : ' + JSON.stringify(_data));
    ...
    sendData(data, linkid = null) {
        ...
        // TeLog.info('----------------' + this.constructor.name + ' ' + linkid + ' : ' + data.cmd);
        // TeLog.info('send : ' + JSON.stringify(data));
        if (this.zlib_open) {
            try {
                var ot = JSON.stringify(data);
                if (ot.length > 512) {
                    ot = zlib.deflateSync(new Buffer(ot));
