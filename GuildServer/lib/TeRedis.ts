...
export class ReList extends ReBase {
    ...
    */
    ...
    private onload(callback, err, reply) {
        ...
        if (!err && reply) {
            for(let data of reply){
                this._value.push(JSON.parse(data));
            }
        }
    ...
    findAll(key: string, findValue: any) {
        let result = [];
        var values = this.value;
        for (var i = 0; i < values.length; i++) {
            if ((!key || values[i][key] == findValue) && !values[i].is_close) {
    ...
    public set(index: number, value) {
        ...
        this.send_command('LSET', [this.key, index, JSON.stringify(value)], this.onError);
