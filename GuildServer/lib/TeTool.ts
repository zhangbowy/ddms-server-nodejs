...
export class TeMap<T>{
    ...
    forEach(cb: (v: T, key: string, map: TeMap<T>) => void) {
        for (let key in this._data) {
            cb(this._data[key], key, this);
        }
    }
...
export function isRandom(probability){
    var odds = Math.floor(Math.random()*10000);  
