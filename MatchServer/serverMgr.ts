...
class ServerMgr {
    ...
    randomServer(type: string, plt: string = '') {

        if (['qzone', 'sdw'].indexOf(plt) >= 0) plt = 'sdw';
        if(configInst.get('openGlobal')) plt = 'global';
        var v_s = this._all_server_.get(type);
        var selpools = [];
        for (var i = 0; i < v_s.length; i++) {
            var r = v_s[i];
            if (r && r.url && liveInst.get_race_count(r.url) > configInst.get("mxrace")) {
                continue;
            }

            let splt = r._sys_.plt;
            if (['qzone', 'sdw'].indexOf(splt) >= 0) splt = 'sdw';
            if(configInst.get('openGlobal')) splt = 'global';
            if (plt && splt != plt) {
                continue;
            }
        ...
        if (selpools && selpools.length > 0) {
            return selpools[Math.floor(selpools.length * Math.random())];
        }
