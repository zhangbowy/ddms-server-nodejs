...
class peakService extends IfMatchServiceBase {
    ...
    //匹配单元粒度
    static peak_score_range = 50;
    //匹配每刷新一次的增量(单位为倍率)
    static peak_range_rate = 1;
    //匹配范围阈值(单位为倍率)
    static peak_range_limit = 3;
    //匹配优先顺序 1 正序 大的优先, -1 反序 小的优先
    static peak_match_order = 1;

    static enter(score: number) {
        return Math.floor(score / this.peak_score_range).toString();
    }
    ...
    static mode = MatchType.match_peak;
    ...
    static match(plt: string, floor: string, get_uids: (floor: string) => number[], reset_pool: (floor: string, uids: number[]) => void) {
        ...
        for (let i = -this.peak_range_limit; i <= this.peak_range_limit; i++) {
            // 找到所有满足进入当前floor上的单位
        ...
        for (let i = 0; i * 2 < match_pool.length - 1; i++) {
            ...
            raceService.create_race(plt, [match_pool[i * 2].uid, match_pool[i * 2 + 1].uid], this.mode, LiveMode.peak);
        }
    }
}

MatchPoolMgr.regist_pool(peakService.mode, peakService);