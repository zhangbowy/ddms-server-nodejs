...
-  Fixed not emitting ready after  with disable_resubscribing ([@maxgalbu](https://github.com/maxgalbu))
...
-  Fixed .multi / .batch used with Node.js 0.10.x not working properly after a  ([@BridgeAR](https://github.com/BridgeAR))
...
- Do not try to  after the connection timeout has been exceeded (@BridgeAR)
...
6. The offline queue is not flushed anymore on a . It'll stay until node_redis gives up trying to reach the server or until you close the connection.
...
* Correctly time  counts when using 'auth'. (William Hockey)
...
* Restore client state on , including select, subscribe, and monitor. (Ignacio Burgueño)
...
* [GH-127] - properly re-initialize parser on 
