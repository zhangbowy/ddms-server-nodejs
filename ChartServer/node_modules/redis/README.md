...
`client` will emit `reconnecting` when trying to  to the Redis server after losing the connection. Listeners
...
* `connect_timeout`: *3600000*; Setting `connect_timeout` limits total time for client to connect and .
...
limits total amount of connection tries. Setting this to 1 will prevent any  tries.
