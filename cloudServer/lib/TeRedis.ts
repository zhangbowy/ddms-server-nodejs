...
class RedisMonit {
    ...
    private noticecb: (type: string, info: string, rate: number) => void;
    constructor() {

    }

    private _updateHandle: NodeJS.Timer = null;

    openWork(conn: IORedis.Redis, noticecb: (type: string, info: string, rate: number) => void) {
        ...
        this._updateHandle = setInterval(this.updateMonit.bind(this), 10000) as NodeJS.Timer;
    ...
    updateMonit() {
        // console.log('query');
        if (!this.redisClient['nodes']) {
            this.redisClient.info(this.onMonitInfo.bind(this));
        }
        else {

            let clients = this.redisClient['nodes']();
            for (let i = 0; i < clients.length; i++) {
                let r = clients[i] as IORedis.Redis;
                r.info(this.onMonitInfo.bind(this));
            }
    ...
    private _parseInfos(res: string): ifRedisInfo {
        if (!res) {
            return null;
        }
        let array = res.split('\r\n');
        ...
        for (let i = 0; i < array.length; i++) {
            let r_wd = array[i];
            if (!r_wd || !r_wd.trim()) continue;
            let shp_idx = r_wd.indexOf('#');
            if (shp_idx != -1) {
                cfgkey = r_wd.slice(shp_idx + 1, r_wd.length).trimLeft().trimRight();
                continue;
            }
            ...
            let cfg_str = r_wd.split(':');
            if (cfg_str.length > 1) {
                use_cfg[cfg_str[0]] = cfg_str[1];
            }

            if (cfg_str[1] == parseFloat(cfg_str[1]).toString()) {
                use_cfg[cfg_str[0]] = parseFloat(cfg_str[1]);
            }
        ...
        return cfgs as ifRedisInfo;
    }

    private _save_interface(cfgs: Object, deep = 0): Object | string {
        let cc = {};
        for (let key in cfgs) {
            let tp = typeof cfgs[key];
            if (cfgs[key] instanceof Array) {
                cc[key] = 'Array<any>';
            }
            else if (tp == 'object') {
                cc[key] = this._save_interface(cfgs[key], deep + 1);
            }
            else {
                cc[key] = tp;
            }
        ...
        if (deep == 0) {
            let cc = this._save_interface(cfgs);
            let str = JSON.stringify(cc, null, 4);
            str = str.replace(/"/g, '');
            str = str.replace(/:/g, '?:');
            return str;
        }
    ...
    onMonitInfo(err: Error, res: string) {
        if (err) {
            console.log(err);
        }
        else if (res) {

            let cfgs = this._parseInfos(res);
            // console.log(cfgs);
            let lessRage = 100;
            if (cfgs.Memory.maxmemory && cfgs.Memory.used_memory) {
                let rt = Math.ceil(cfgs.Memory.used_memory / cfgs.Memory.maxmemory * 100);
                lessRage = lessRage - rt;
            }

            // console.log(cfgs.Server.config_file, lessRage);
            if (lessRage < 10) {
                this.noticecb && this.noticecb("dbless", cfgs.Server.config_file, lessRage);
            }

            this.db_lessRage = lessRage;
            // writeFileSync('./redis.txt', str, { flag: "w+" });
        }
    }
}
...
export class TeRedis extends EventEmitter {
    ...
    private _monit: RedisMonit;


    get db_lessrate() {
        return this._monit.db_lessRage
    }

    get is_login_forbid() {
        return this.db_lessrate < 5
    }

    constructor() {
        super();
        this._monit = new RedisMonit()
    }
    ...
    init(ports: IORedis.ClusterNode[], dbnum, flags) {
        ...
        this.on('ready', () => {
            this._monit.openWork(this._redisClient, (type, info, rate) => {
                this.emit('dbmonit', type, info, rate);
            })
        });
    ...
    public getHash(key: string, bindClass?: Function): ReHash<any> {
    ...
    public getList(key: string): ReList<any> {
...
// 列表应该使用的比较少
export class ReList<T> extends ReBase {
    constructor(redisClient: TeRedis, key) {
        super(redisClient, key);
        this._value = [];
        this._isArray = true;
    ...
    */
    ...
    clearAll() {
        this.redisClient.ltrim(this.key, this._value.length, -1, this.onError);
        // this.send_command('LREM', [this.key, 1, value], this.onError);
        this._value = [];
    }
}
...
export class ReHash<T> extends ReBase implements ifReHash {
    ...
    */
    ...
    public msave(a: Array<{ k: string, v: any }>) {
        ...
        if (configInst.get("cheatmode")) {
            return this.msave2(a);
        }
        else {
            return this.msave1(a);
        }
    }
    ...
    public save(key: string, value: any): boolean {
        this._curr_v_++;
        if (configInst.get("cheatmode")) {
            return this.msave2([{ k: key, v: value }]);
        }
        else {
            return this.msave1([{ k: key, v: value }]);
        }
    }
    ...
    public del(key: string | string[]): any {
        ...
        if (configInst.get("cheatmode")) {
            return this.del2(key);
        }
        else {
            return this.del1(key);
        }
    }

    private _cache: string[] = [];

    private _save_caches_() {
        let savelist = [this.key];
        let dellist = [this.key];
        for (let i = 0; i < this._cache.length; i++) {
            let key = this._cache[i];
            let value = this._value[key];
            if (value == undefined) {
                dellist.push(key);
            }
            else {
                savelist.push(key, value);
            }
        }

        if (savelist.length > 1) {
            this.send_command('hmset', savelist, this.onError);
        }

        if (dellist.length > 1) {
            this.send_command('hdel', dellist, this.onError);
        }

        this._cache = [];
        this._save_handle = null;
    }

    private _save_handle;
    private _check_save_() {
        if (this._cache.length == 0) return;
        if (this._save_handle) return;

        setTimeout(this._save_caches_.bind(this), 1000);
    }

    private msave2(a: Array<{ k: string, v: any }>) {
    ...
    private del2(keys: string[]): any {
        for (var i = 0; i < keys.length; i++) {
            var key = JSON.stringify(keys[i]);
    ...
    private msave1(a: Array<{ k: string, v: any }>) {
        var list = [this.key];
        for (var i = 0; i < a.length; i++) {
            var value = JSON.stringify(a[i].v);
            var key = JSON.stringify(a[i].k);
    ...
    private del1(key: string[]): any {
        var delKeys = [this.key];
        ...
        this.send_command('hdel', delKeys, this.onError);
    }

    public get(key: string): any {
        if (this._cache_v_ && this._cache_v_ == this._curr_v_) {
            return this._value_cache[key];
        }
        else {
            key = JSON.stringify(key);
            if (this._value.hasOwnProperty(key)) {
    ...
    private _value_cache: any = {};
    /**数据缓冲池，如果初始值是0的话就不使用缓存 */
    private _cache_v_: number = 1;
    private _curr_v_: number = 1;
    get value() {
        if (!this._cache_v_ || this._cache_v_ != this._curr_v_) {
            var out = {};
            for (var key in this._value) {
            ...
            if (!this._cache_v_) return out;

            this._cache_v_ = this._curr_v_;
            this._value_cache = out;
        }

        return this._value_cache;
    }
}
