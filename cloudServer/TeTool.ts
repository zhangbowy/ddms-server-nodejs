...
export class TeDate extends Date {
    ...
    /**yyyy-MM-dd DD:mm:ss */
    static DateToStr(pdate: Date | number) {
    ...
    /**yyyyMMdd */
    static DateToyyyyMMdd(pdate: Date | number) {
        if (typeof pdate == 'number') {
            pdate = new Date(pdate);
        }
        else if (typeof pdate == 'string') {
    ...
    /**yyyyMMdd_hh */
    static DateToLogStr(pdate: Date | number) {
        if (typeof pdate == 'number') {
            pdate = new Date(pdate);
        }
        else if (typeof pdate == 'string') {
    ...
    static todays(time:number){
        return Math.floor(time / (24 * 60 * 60 * 1000));
    }

    static toWeeks(time:number): number{
     ...
     * @param ntime 
     */
    static isInTime(leftTime: number, rightTime: number, ntime: number = Date.now()): Boolean {
        let curr = new Date();
...
export function isRandom(probability){
 ...
 */
export function arrayRandomT<T>(a: Array<T>, weight_key: string, remove = false): T {
    var total = 0;
...
export class HashMap<T>{
    ...
    has(key: string | number, v: T = null) {
        key = key.toString();
...
export function getCharByte(str: string): number {
    var totalCount = 0;
...
export function formationName(str: string): string {
    // 先洗掉表情符号
    ...
    if (i == str.length && totalCount <= 12) return str;
    return str.substr(0, i) + "…";
}

export function isChinese(c: number): boolean {
 ...
 */
export function getProbabilityResult(weightArr: Array<number>): number {
 ...
 * @param dst_obj 
 */
export function isKeyEqual(src_obj: object, dst_obj: object) {
...
export function utf8StringLength(inputStr: string) { 
    var totalLength = 0;
...
export function XiaoDuSign(params) {
    //所有参数按照参数名称字母顺序排序
    var keys = [];
    ...
    let signValue = createHash('SHA256').update(s2).digest('hex');;
    return signValue;
}
...
//数组push元素并去重
export function remove_duplicate(ins:SeItem, outs:SeItem[]) {
    let duplicate = false;
...
//数组push元素并去重
export function remove_duplicatev2(ins:any, outs:Array<any>, equals_key, add_key) {
    let duplicate = false;
    for(let i = 0; i < outs.length; i++){
