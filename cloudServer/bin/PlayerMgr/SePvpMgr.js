...
const MAX_COUNT = 20;
const Box_Max = 4;
const BOX_KEY_PRICE = 1; //1沙漏=10钻石
class SePvpMgr {
    constructor(parent) {
        //------------PvP v726 begin------------------------//
        this.match_score = 0;
    ...
    test(score) {
        global.globalChartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PEAK_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level }, this.peak_score);
    }
    set peak_score(score) {
        //通知榜单服
        //SCORE_INIT_VALUE分数不需要通知
        if (this.peak_score <= SeDefine_1.SCORE_INIT_VALUE && score <= SeDefine_1.SCORE_INIT_VALUE) {
            global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_PEAK_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level }, this.peak_score);
        }
        this._baseInfo.peak_score = score;
        this._parent.taskAction(SeDefine_1.TaskAction.AddScore, score);
        // 操作完成后必须立即保存一次数据库
        this.saveInfo('peak_score');
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'peak_score', this.peak_score);
    }
    set shangjin_score(score) {
        //通知榜单服
        //SCORE_INIT_VALUE分数不需要通知
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'shangjin_score', this.shangjin_score);
    }
    ...
    set max_pve_pk_rank(n) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "max_pve_pk_rank", this._baseInfo.max_pve_pk_rank);
    }
    set pvp_score(n) {
        n = Math.floor(n);
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "pvp_score", this._baseInfo.pvp_score);
    }
    set pvp_wincount(n) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "winStreakCount", this._baseInfo.win_count);
    }
    /**1v1连续胜场数 */
    get pvp_wincount() {
    ...
    set pvp_top_wincount(n) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "topwinStreakCount", this._baseInfo.top_win_count);
    }
    /**1v1 最高胜场数 */
    ...
    set pvp_win2v2count(n) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "winStreak2v2Count", this._baseInfo.win_2v2_count);
    }
    get pvp_win2v2count() {
    ...
    set pvp_top_win2v2count(n) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "topwinStreak2v2Count", this._baseInfo.top_win_2v2_count);
    }
    ...
    set pve_pk_fight_time(time) {
        this._baseInfo.pve_pk_fight_time = time;
        this.saveInfo('pve_pk_fight_time');
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "pve_pk_fight_time", this._baseInfo.pve_pk_fight_time);
    }
    get glory_score() {
    ...
    set glory_score(s) {
        ...
        if (s > 0) {
            global.chartMgr.addPlayerLevelChart(this.seasonid, interface_1.SeEnumChartTableeType.DanFuRongYaoJiFenBang, {
                id: this._parent.id,
            ...
            global.globalChartMgr.addPlayerLevelChart(this.seasonid, interface_1.SeEnumChartTableeType.QuanFuRongYaoJiFenBang, {
                id: this._parent.id,
                name: this._parent.name,
                score: this._baseInfo.glory_score,
                icon: this._parent.icon,
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "glory_score", this._baseInfo.glory_score);
    ...
    set glory_score_all(s) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "glory_score_all", this._baseInfo.glory_score_all);
    ...
    set glory_kill(s) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "glory_kill", this._baseInfo.glory_kill);
    ...
    set glory_kill_all(s) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, "glory_kill_all", this._baseInfo.glory_kill_all);
        this.saveInfo("glory_kill_all");
    }
    set top_pvp_rank(val) {
        if (val == null || val == undefined)
            return;
        if (!this._baseInfo)
            return;
        if (!this._baseInfo.top_pvp_rank || val < this._baseInfo.top_pvp_rank) {
            ...
            if (this._parent.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "top_pvp_rank", this._baseInfo.top_pvp_rank);
        }
    }
    get pvp_level() {
    ...
    cheat_pvp_level(v) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'pvp_star', this.pvp_star, this.pvp_level);
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'top_pvp_level', this.top_pvp_level, []);
        var pkBattleRank = global.resMgr.getBattleRankByLevel(this.pvp_level);
        if (pkBattleRank && pkBattleRank.eProperty & interface_1.SeEnumBattleRankeProperty.ZuiZhongCeng) {
            // 打仗打好了，看看自己的段位，如果是白金了那么通知榜单服务器
            global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_PVP_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.pvp_score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
            if (TeConfig_1.configInst.get('globalCsMgr.url') && global.resMgr.getConfig("1v1_cross_server") == 'true') {
                global.globalChartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PVP_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.pvp_score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
            }
            if (this.groupId) {
                global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GROUP_PVP_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.pvp_score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
            }
            global.chartMgr.queryranks(this.seasonid, this._parent.id, SeDefine_1.SeChartType.SCT_PVP_SCORE);
        }
    }
    get top_pvp_level() {
    ...
    _star_act(level, star, addstar) {
        var out = { level: level, star: star };
        var pkBattleRank = global.resMgr.getBattleRankByLevel(level);
        if (!pkBattleRank || addstar == 0)
            return out;
        if (addstar > 0 && !(pkBattleRank.eProperty & interface_1.SeEnumBattleRankeProperty.ZuiZhongCeng)) {
            if (addstar + star > pkBattleRank.iStarNum) {
                var pkBattleRankb = global.resMgr.getBattleRankByLevel(level);
                if (!pkBattleRankb)
                    out.star = pkBattleRank.iStarNum;
                else
                    out = this._star_act(level + 1, 0, addstar + star - pkBattleRank.iStarNum);
            }
            else {
            ...
            if (addstar + star < 0) {
                if ((pkBattleRank.eProperty & interface_1.SeEnumBattleRankeProperty.JiangDuanBaoHu || this._parent.m_buffMgr.isHadBuff('B006') && interface_1.SeEnumTownBuffereType.JiangDuanBaoHu)) {
                    out.star = 0;
                }
                else {
    ...
    set pvp_star(n) {
        ...
        if (oo.level > this._baseInfo.pvp_level) {
            ...
            if (oo.level == global.resMgr.MaxLvl) {
                this._parent.sendAnnouncement(interface_1.SeEnumnoticetexteType.ShengDuoWang, { pvplevel: oo.level }, this._parent);
            }
            if (pkNextBattleRank.akRewards) {
                let items = [];
                for (let i = 0; i < pkNextBattleRank.akRewards.length; i++) {
                    if (!pkNextBattleRank.akRewards[i])
                        continue;
        ...
        if (this._baseInfo.pvp_level > this._baseInfo.top_pvp_level) {
            ...
            let cht_unlock = global.resMgr.getUnlockInfo(interface_1.SeEnumUnlockeFunc.PaiXingBang);
            if (!this.groupId && !this.groupState && cht_unlock && this._baseInfo.top_pvp_level >= cht_unlock.iOpengrade) {
                // 这里处理一下跳过引导的玩家的榜单问题
                this.groupState = 'new';
                global.chartMgr.apply_group_id(this.seasonid, SeDefine_1.SeChartType.SCT_GROUP_PVP_SCORE, this._parent.id, this.groupState);
            }
            var items = [];
            if (pkNextBattleRank.akAwards && this._parent.baseInfo.share_uid) {
                ...
                global.playerMgr.onGiveMail(this._parent.plt, this._parent.baseInfo.share_uid, SeDefine_1.SeMailType.AutoUse, '', items);
            }
            if (this._parent.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'top_pvp_level', this.top_pvp_level, items);
        }
    }
    //-------------------------------------------------//
    /**
    ...
    //-------------------------------------------------//
    initDbInfo(obj) {
        this._baseInfo = new SePlayerDef_1.SePvpInfo(obj);
        var needSave = false;
    ...
    on_match_pvp(opp, mode, rmode) {
        ...
        this._parent.setState(SeDefine_1.CharState.inrace);
     ...
     * @param tTime
     */
    completeFight(isWin, tTime, isBossDie) {
    ...
    accOpenBox(index) {
        if (this._baseInfo.boxList && this._baseInfo.boxList[index]) {
            ...
            if (boxInfo.ftime > 0 && boxInfo.ftime < Date.now())
                return;
            if (boxInfo.optime <= 0)
                return;
            //限制
    ...
    upgradeBox(boxid) {
        ...
        var dec_time_buff = this._parent.get_buffer_value(interface_1.SeEnumTownBuffereType.XiangZiJiaSu);
        //vip减少宝箱时间
        ...
        for (let i = 0; i < box.length; i++) {
            ...
            if (r_box && r_box.id == boxid) {
                ...
                if ((!r_box.up_count || r_box.up_count < 1)
                    && (!r_box.ftime)
                    && ((r_box.type >= interface_1.SeEnumHeroBoxTypeeType.YuanMuBaoXiang && r_box.type < interface_1.SeEnumHeroBoxTypeeType.MiYinBaoXiang)
                        || (r_box.type >= interface_1.SeEnumHeroBoxTypeeType.XunLianBaoXiang && r_box.type < interface_1.SeEnumHeroBoxTypeeType.XunLianHuangJin))) {
    ...
    completeOpenBox(index) {
        if (this._baseInfo.boxList && this._baseInfo.boxList[index]) {
            ...
            if (time > 0) {
                if (!this.useBoxKey(oprbox.level, oprbox.type, time, (oprbox.org_time || oprbox.optime) * 60 * 1000))
                    return;
            }
            var boxLevel = level;
     ...
     * @param time 剩余时间
     */
    _getOpenPrice(level, type, time, org_time) {
    ...
    useBoxKey(level, type, time, org_time) {
        ...
        for (var i = 0; items && i < items.length; i++) {
            ...
            if (!pkItemRes)
                continue;
            if (pkItemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.YueChi) {
        ...
        if (itemNum < cost) {
            ...
            if (!this._parent.decMoney(needMoney, "buyBoxKey"))
                return false;
            if (itemNum > 0)
                this._parent.delItem(itemId, itemNum, SePlayerDef_1.DeAddDelItemReason.buyBoxKey);
        }
        else {
            this._parent.delItem(itemId, cost, SePlayerDef_1.DeAddDelItemReason.buyBoxKey);
        }
        return true;
    }
    /**
    ...
    addNewBox(type, level, num) {
        ...
        for (var j = 0; j < num; j++) {
            isFull = true;
            var box = new SePlayerDef_1.SePvpBox();
    ...
    _addFightAward(pvpLevel, bRate = false) {
        ...
        if (pkBattleRank && pkBattleRank.akSequence && pkBattleRank.akSequence.length > 0) {
            var kSequince = pkBattleRank.akSequence[index % pkBattleRank.akSequence.length].toString();
    ...
    _is_fresh_protected() {
        if (this.pvp_level <= 3) {
            //新手引导的三把里面没有连赢两把
            if (this._baseInfo.fight_count < 3 && this.pvp_wincount < 2)
                return true;
            //连输两把以上
            if (this.pvp_losecount >= 2)
                return true;
        }
        return false;
    }
     ...
     */
    on_enter_match(mode, gamekey, isCross) {
        if (this.raceInfo) {
        ...
        this._parent.setState(SeDefine_1.CharState.matching);
        ...
        if (mode == '1v1') {
            ...
            // 1v1 的时候增加连胜进去
            // 增加连败和历史对手
            if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("1v1_cross_server") == 'true' && isCross) {
            ...
            // 2v2 使用平衡模式
            if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("2v2_cross_server") == 'true') {
            ...
            if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("peak_cross_server") == 'true' && isCross) {
            ...
            if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("sjs_cross_server") == 'true') {
            ...
            if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("infinite_cross_server") == 'true') {
    ...
    // 匹配包括人机和非人机两种
    on_match_pvp_v726_ms(result, randomList, mode, rmode) {
        // 玩家匹配结果返回
        this.raceInfo = { type: "offline", mode: '1v1', pvp_score: result.pvp_score, Name: result.Name, extra: undefined };
        this.mode = '1v1';
        this._raceType = SeDefine_1.EnumRaceType.Pvp726;
        this._parent.setState(SeDefine_1.CharState.inrace);
        global.netMgr.sendPlayerPvPv726RaceBegin(this._parent.linkid, result, null, randomList, mode, rmode);
    }
    // 匹配包括2v2人机
    on_match_pve_2v2_ms(result, randomList) {
        // 玩家匹配结果返回
        var pes2 = Math.floor(Math.pow(TeTool_1.TeMath.stdev(result[2].pvp_score, result[3].pvp_score), 1 + 0) + Math.min(result[2].pvp_score, result[3].pvp_score));
        this.mode = '2v2';
    ...
    // 匹配包括1v2人机
    on_match_pve_1v2_ms(result, randomList, mode, rmode, formation) {
        // 玩家匹配结果返回
        this.mode = '1v2';
    ...
    add_protect_score(bWin) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'pvp_protect_score', this._baseInfo.pvp_protect_score);
        return add;
    }
    /**
     * 人机下的结算流程
     ...
     * @param rid
     */
    on_match_pve_fight(bWin, tTime, isBossDie, rid, isCross, times) {
    ...
    _is_limit_time(check_level = true) {
        let res = global.resMgr.getBattleRankByLevel(this.pvp_level);
        if (check_level && (res.eProperty & interface_1.SeEnumBattleRankeProperty.ZuiZhongCeng) != interface_1.SeEnumBattleRankeProperty.ZuiZhongCeng) {
            // 夺王之前没有修养期
            return false;
        }
     ...
     * 闪电玩比赛验证
     */
    make_sync_key(bwin, up_score, rid, boxId) {
        ...
        game_stamp_info = {
            ...
            win: matchResult,
            rid: rid,
            stmp: crypto_1.createHash('md5').update(this._parent.id.toString() + this.gamekey.toString() + matchResult.toString() + cttime.toString() + 'baiwanhongbao').digest('hex'),
    ...
    on_match_pvp_v726_fight(bWin, tTime, isBossDie, pvpscore, pve, rid, oppName, opplevel, isCross, is_robot) {
        tTime = Math.max(tTime, 1000);
        ...
        if (bWin) {
            ...
            if (!gloryLimit) {
                ...
                if (isCross && !is_robot && this.pvp_level >= 16) {
                    ...
                    let add = Math.log(Math.max(this.pvp_score, 3000) - 2000) / Math.log(10) - fixed;
            ...
            if (this._baseInfo.win_count >= 10) {
                this._parent.sendAnnouncement(interface_1.SeEnumnoticetexteType.LianSheng, { charname: this._parent.name, win_count: this._baseInfo.win_count }, this._parent);
            }
            if (!bLimit)
                add_star += 1;
            this.pvp_star += add_star;
            let pre_id = this._baseInfo.genBoxID;
            if (!this.gamekey || this.gamekey == '')
                items = this._addFightAward(this._baseInfo.pvp_level);
            ...
            if (this._baseInfo.win_count >= 10) {
                this._parent.sendAnnouncement(interface_1.SeEnumnoticetexteType.JuJiLianSheng, { oppname: oppName, charname: this._parent.name, win_count: this._baseInfo.win_count }, this._parent);
            }
            this.pvp_wincount = 0;
            this.pvp_losecount++;
            //提示购买宝箱,23点50分之前才触发
            ...
            if (battleRes) {
                if (battleRes.eProperty & interface_1.SeEnumBattleRankeProperty.ZuiZhongCeng) {
                    // 最终层不需要扣道具
                    decCount = 0;
                }
                else if ((battleRes.eProperty & interface_1.SeEnumBattleRankeProperty.JiangDuanBaoHu) && this.pvp_star == 0) {
                    // 降低段保护的 需要保护
                    decCount = Math.min(Math.abs(this.pvp_star), decCount);
                }
                else if (this._parent.m_buffMgr.isHadBuff('B006') && interface_1.SeEnumTownBuffereType.JiangDuanBaoHu && this.pvp_star == 0) {
                    // 降低段保护的 需要保护
                    decCount = Math.min(Math.abs(this.pvp_star), decCount);
        ...
        this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.Pvp726, 1, (isCross && this.pvp_level >= 16) ? 1 : 0);
        if (isBossDie)
            this._parent.taskAction(SeDefine_1.TaskAction.FightBossKill, bWin, SeDefine_1.EnumRaceType.Pvp726, 1);
        this._parent.taskAction(SeDefine_1.TaskAction.FightJoin, race_type_log, bWin);
        ...
        for (let i in chartTable) {
            if (chartTable[i].eType == interface_1.SeEnumChartTableeType.DanPiJiFen) {
            ...
            else if (chartTable[i].eType == interface_1.SeEnumChartTableeType.ZhanQuBangDan) {
                if (chartTable[i].iOpenGrade <= this.pvp_level) {
                    if (this.groupId) {
                        global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GROUP_PVP_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.pvp_score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
                    }
            ...
            else if (chartTable[i].eType == interface_1.SeEnumChartTableeType.PaiWeiLianSheng) {
                if (chartTable[i].iOpenGrade <= this.pvp_level) {
                    if (this.pvp_top_wincount >= 2) {
                        global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_1V1_WIN, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.pvp_top_wincount, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
                    }
                }
            }
        }
    ...
    send_win_count_sdw() {
        ...
        let sign = crypto_1.createHash('md5').update(appid).update(channel).update(uid).update(sid).update(level).update(type).update(vip).update(power).update(appKey).digest('hex');
    ...
    on_match_pvp_fight_2v2(bWin, tTime, isBossDie, rid, bTeam) {
        ...
        if (bWin) {
            ...
            if (!this.gamekey || this.gamekey == '')
                items = this._addFightAward(this._baseInfo.pvp_level, bExt);
            if (pre_id != this._baseInfo.genBoxID) {
                addBoxId = pre_id;
            }
        ...
        this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.Pvp726, 2);
        if (isBossDie)
            this._parent.taskAction(SeDefine_1.TaskAction.FightBossKill, bWin, SeDefine_1.EnumRaceType.Pvp726, 2);
        this._parent.taskAction(SeDefine_1.TaskAction.FightJoin, race_type_log, bWin);
        global.logMgr.fightFormationLog(this._parent, race_type_log.toString(), bWin, rid || '', tTime, -1, this._parent.baseInfo.lord, this._parent.m_equipMgr.getLordEquip(), -1, '', false, false, [], ...this._parent.getLogFormation());
        var chartTable = global.resMgr.chartTable.getAllRes();
        for (let i in chartTable) {
            if (chartTable[i].eType == interface_1.SeEnumChartTableeType.ZuDuiLianSheng) {
                if (chartTable[i].iOpenGrade <= this.pvp_level) {
                    if (this.pvp_top_win2v2count >= 2) {
                        global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_2V2_WIN, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.pvp_top_win2v2count, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
                    }
                }
            }
        }
    ...
    on_match_pvp_fight_peak(bWin, tTime, isBossDie, oppscore, pve, rid, oppName, isCross) {
        ...
        if (bWin) {
            ...
            //保护分
            if (_peak_score < SeDefine_1.SCORE_INIT_VALUE) {
                _peak_score = SeDefine_1.SCORE_INIT_VALUE;
            }
        ...
        if (isBossDie)
            this._parent.taskAction(SeDefine_1.TaskAction.FightBossKill, bWin, SeDefine_1.EnumRaceType.Pvp726, 3);
        this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.peakPvp, 1);
        this._parent.taskAction(SeDefine_1.TaskAction.FightJoin, race_type_log, bWin);
        global.logMgr.fightFormationLog(this._parent, race_type_log.toString(), bWin, rid || '', tTime, -1, this._parent.baseInfo.lord, this._parent.m_equipMgr.getLordEquip(), -1, '', false, false, [], ...this._parent.getLogFormation());
        var chartTable = global.resMgr.chartTable.getAllRes();
        for (let i in chartTable) {
            if (chartTable[i].eType == interface_1.SeEnumChartTableeType.DianFengBang) {
                if (chartTable[i].iOpenGrade <= this.pvp_level) {
                    // 打仗打好了，看看自己的段位，如果是白金了那么通知榜单服务器
                    global.chartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_PEAK_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.peak_score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
                    if (TeConfig_1.configInst.get('globalCsMgr.url') && global.resMgr.getConfig("peak_cross_server") == 'true') {
                        global.globalChartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PEAK_SCORE, { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this.peak_score, icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId ? this.groupId : this._parent.plt, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level });
                    }
    ...
    //赏金赛结算
    on_match_pvp_fight_shangjin(bWin, tTime, isBossDie, oppscore, pve, rid, oppName) {
        ...
        this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.Pvp726, 4);
        this._parent.taskAction(SeDefine_1.TaskAction.FightJoin, race_type_log, bWin);
        global.logMgr.fightFormationLog(this._parent, race_type_log.toString(), bWin, rid || '', tTime, -1, this._parent.baseInfo.lord, this._parent.m_equipMgr.getLordEquip(), -1, '', false, false, [], ...this._parent.getLogFormation());
        // var chartTable = global.resMgr.chartTable.getAllRes();
        // for (let i in chartTable) {
    ...
    on_match_1v2_fight(bWin, tTime, isBossDie, rid, host_id, host_skills, times, index, formation_1v2) {
        ...
        //有豆子变动的才算任务，排除训练场
        beans_count && this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.Pvp726, 5);
        beans_count && this._parent.taskAction(SeDefine_1.TaskAction.FightJoin, race_type_log, bWin);
    ...
    //诸侯伐董结算
    on_match_pve_pk(bWin, tTime, opp_info, fight_low) {
        //榜单已经超时的不结算
        let res = global.resMgr.getChartTableByType(SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE);
        let curr = Date.now();
        if (res.kStartDate && curr < Date.parse(res.kStartDate))
            return;
        if (res.kEndDate && curr > Date.parse(res.kEndDate))
            return;
        var rank_change = 0;
        ...
        for (var key in opp_pve_pk_formation) {
            if (key == 'lord')
                opp_equips = opp_pve_pk_formation[key].equips;
            else
                opp_formation.push({ kHeroId: opp_pve_pk_formation[key].heroId, iLevel: opp_pve_pk_formation[key].level });
        }
        for (var key in this._parent.baseInfo.pve_pk_formation[0]) {
            if (key == 'lord')
                equips = this._parent.baseInfo.pve_pk_formation[0][key].equips;
            else
                formation.push({ kHeroId: this._parent.baseInfo.pve_pk_formation[0][key].heroId, iLevel: this._parent.baseInfo.pve_pk_formation[0][key].level });
        }
        ...
        if ((player_score / opp_score) < Number(global.resMgr.getConfig('JJC_cheat').split(',')[0]) && !TeConfig_1.configInst.get("cheatmode")) {
        ...
        if (!fight_low && this.pve_pk_rank > opp_info.score) {
            if (bWin) {
                if (this._baseInfo.pve_pk_rank > opp_info.score) {
                    ...
                    opp_info.score = temp;
                    global.globalChartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE, opp_info);
                    global.globalChartMgr.addPlayerLevelChart(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE, { seasonid: this.seasonid,
                        id: this._parent.id,
                        name: this._parent.name,
                    ...
                    this._parent.taskAction(SeDefine_1.TaskAction.PvePkRank, SeDefine_1.PVE_PK_INIT_VALUE - this._baseInfo.pve_pk_rank);
                    this.max_pve_pk_rank = SeDefine_1.PVE_PK_INIT_VALUE - this._baseInfo.pve_pk_rank;
                    //记录
                ...
                let lengthMax = global.resMgr.getChartTableByType(SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE).iMaxPlayer;
                if (global.playerMgr.pve_pk_length < lengthMax)
                    global.globalChartMgr.sendCSData({ cmd: 'check_last_one', charid: this._parent.id, opp_name: opp_info.name, season_id: this.seasonid, type: SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE, info: { seasonid: this.seasonid, id: this._parent.id, name: this._parent.name, score: this._baseInfo.pve_pk_rank, pve_pk_formation: this._parent.baseInfo.pve_pk_formation[1] ? this._parent.baseInfo.pve_pk_formation[1] : [], icon: this._parent.icon, avatar: this._parent.avatar, igroup: this.groupId, is_vip: this._parent.baseInfo.is_vip, vip_level: this._parent.baseInfo.vip_level } });
            }
        }
    ...
    simpleChangePveRank(data) {
        if (!data.rank || data.rank == this._baseInfo.pve_pk_rank)
            return;
        this._baseInfo.pve_pk_rank = data.rank;
        this.saveInfo(["pve_pk_rank"]);
        if (this._parent.bInitComplete) {
    ...
    /**无限火力结算 */
    on_match_pvp_wuxianhuoli(bWin, tTime, isBossDie, rid) {
        tTime = Math.max(tTime, 1000);
        this._baseInfo.fight_count++;
        //记录历史对手
        ...
        this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.Pvp726, 1);
        if (isBossDie)
            this._parent.taskAction(SeDefine_1.TaskAction.FightBossKill, bWin, SeDefine_1.EnumRaceType.Pvp726, 1);
        this._parent.taskAction(SeDefine_1.TaskAction.FightJoin, race_type_log, bWin);
        global.logMgr.fightFormationLog(this._parent, race_type_log.toString(), bWin, rid || '', tTime, -1, this._parent.baseInfo.lord, this._parent.m_equipMgr.getLordEquip(), -1, '', false, false, [], ...this._parent.getLogFormation());
    ...
    pve_pk_refresh(indexs, no_check = false, use_money = false) {
        if (use_money) {
            //如果用钻石刷
            let count = parseInt(global.resMgr.getConfig('JJC_refresh').split('|')[1]);
            if (!this._parent.decMoney(count, 'pve_pk_refresh'))
                return;
            no_check = true;
        }
        //十分钟冷却
        ...
        if (indexs.length == 1)
            no_check = true;
        if (!no_check && (this._baseInfo.pve_pk_refresh_time + internal * 60 * 1000) > Date.now())
            return;
        if (indexs.length == 1) {
            let except = [];
    ...
    checkFight(opp_id, opp_igroup, index) {
        ...
        //10分钟内只能打一次
        if (!TeConfig_1.configInst.get("cheatmode") && (Date.now() - this._baseInfo.pve_pk_fight_time) < 5 * 60 * 1000) {
    ...
    pve_pk_refresh_ret(result) {
        for (var key in result) {
            if (result[key] == {})
                this._baseInfo.pve_pk_opp.splice(parseInt(key), 1);
            else {
    ...
    setRaceID(v, mode = '1v1') {
        if (v) {
            this._parent.setState(SeDefine_1.CharState.inrace);
        }
        else {
            this._parent.setState(SeDefine_1.CharState.loadcomplete);
        }
        this._raceID = v;
        this.mode = mode;
    }
    ...
    get_target_score(sa, sb) {
        var k = 0;
        var s = Math.pow(TeTool_1.TeMath.stdev(sa, sb), 1 + k) + Math.min(sa, sb);
     ...
     */
    on_match_pvp_ol_back(rid, checkKey, rurl, oscore, mode) {
        // 
        ...
        this._raceType = SeDefine_1.EnumRaceType.olPvp;
        // 这里需要通知玩家链接比赛服
     ...
     * @param isUnSync
     */
    onMatchPvpOlFight_MS_BACK(bWin, time, isBossDie, mode, pvp_score, rmode, rid, bTeam, oppname, state, isUnSync, isCross, opplevel, host_id, host_skills, times, index, formation_1v2) {
        if (isUnSync) {
            // 不同步的录像记录一个日志
            let raceType = 1022;
            if (mode == "peakmatch") {
        ...
        if (state == 'cancell') {
            ...
            this.setRaceID(null);
            // if(mode != 'peakmatch') {
            this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.olPvp, this.mode == '2v2' ? 2 : 1);
            // }
        }
        else {
            if (mode == '1v1')
                this.on_match_pvp_v726_fight(bWin, time, isBossDie, pvp_score, false, rid, oppname, opplevel, isCross, false);
            else if (mode == 'peakmatch') {
                this.on_match_pvp_fight_peak(bWin, time, isBossDie, pvp_score, false, rid, oppname, isCross);
            }
            else if (mode == 'shangjinmatch') {
                this.on_match_pvp_fight_shangjin(bWin, time, isBossDie, pvp_score, false, rid, oppname);
            }
            else if (mode == 'wuxianhuoli') {
                this.on_match_pvp_wuxianhuoli(bWin, time, isBossDie, rid);
            }
            else if (mode == '1v2') {
                this.on_match_1v2_fight(bWin, time, isBossDie, rid, host_id, host_skills, times, index, formation_1v2);
            }
            else {
                this.on_match_pvp_fight_2v2(bWin, time, isBossDie, rid, bTeam);
                if (bTeam) {
                    this._parent.taskAction(SeDefine_1.TaskAction.FightComplete, bWin, SeDefine_1.EnumRaceType.olPvp, 2);
                }
            }
        }
        this.raceInfo = null;
    ...
    onLoginDecBoxTime() {
        // 检查自己是否有特权
        if (!this._parent.get_buffer_value(interface_1.SeEnumTownBuffereType.BaoXiangLiXianJieSuo) && global.resMgr.getConfig('offline_open_box') == 'true') {
            return;
        }
        // 查看上次离线时间和这次登陆时间的时间差
        var timeDiff = this._parent.baseInfo.loginTime - this._parent.baseInfo.lastLogoutTime;
        if (!this._parent.baseInfo.lastLogoutTime)
            timeDiff = 0;
        if (!timeDiff)
            return;
        if (this._baseInfo.boxList) {
            ...
            for (var i = 0; i < Box_Max; i++) {
                ...
                if (!rBox || i == this._baseInfo.openingIndex || (rBox.ftime && rBox.ftime < Date.now()))
                    continue;
                descList.push({ i: i, t: rBox.optime || 0 });
            }
            if (descList.length == 0 || timeDiff <= 0)
                return;
    ...
    _winLog(mode, isWin, isKillBoss) {
        if (mode == "1v1") {
            ...
            if (this._parent.bInitComplete) {
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "log_1v1", this._baseInfo.log_1v1);
            }
            ...
            if (this._parent.bInitComplete) {
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "log_2v2", this._baseInfo.log_2v2);
            }
            return this._baseInfo.log_2v2;
        }
        else if (mode == "wuxianhuoli") {
            if (!this._baseInfo.log_wuxianhuoli) {
                this._baseInfo.log_wuxianhuoli = new SePlayerDef_1.PvpLogInfo();
            }
            ...
            if (this._parent.bInitComplete) {
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "log_wuxianhuoli", this._baseInfo.log_wuxianhuoli);
            }
            ...
            if (this._parent.bInitComplete) {
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "log_peak", this._baseInfo.log_peak);
            }
            ...
            if (this._parent.bInitComplete) {
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "log_pelog_shangjinak", this._baseInfo.log_shangjin);
            }
            ...
            if (this._parent.bInitComplete) {
                global.netMgr.sendCharMiscUpdate(this._parent.linkid, "log_1v2", this._baseInfo.log_1v2);
            }
    ...
    //--------------赛季的功能在这里添加------------------------//
    _get_award_list(pkSeasonRes) {
        if (!pkSeasonRes)
            return null;
        for (var i = pkSeasonRes.akParseAwards.length - 1; i >= 0; i--) {
            var r = pkSeasonRes.akParseAwards[i];
            if (r.elo && this.pvp_score < r.elo)
                continue;
            if (r.pvplevel && this.pvp_level < r.pvplevel)
                continue;
            return r;
        }
        return null;
    ...
    //作弊命令，测试跨服争霸赛奖励
    cheat_test_peak() {
        if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("peak_cross_server") == 'true') {
            global.globalChartMgr.onGiveSeasonCrossReward(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PEAK_SCORE, this._parent.id, this.peak_score);
        }
    }
    /**
   ...
   */
    checkSeason(iLoop = 0) {
        // 赛季结算的解锁
        let luck_info = global.resMgr.getUnlockInfo(interface_1.SeEnumUnlockeFunc.SaiJi);
        ...
        if (pkRes && pkRes.kEndTime &&
            ...
            if (!luck_info || this.pvp_level >= luck_info.iOpengrade) {
                ...
                if (this._parent.baseInfo.is_vip && this._parent.baseInfo.vip_level > 0) {
                    ...
                    if (vip_res && vip_res.akRewardSeason) {
                        ...
                        if (items.length > 0) {
                            let content = TeTool_1.LangID(vip_res.akSeasonMail[0]);
                            let title = TeTool_1.LangID(vip_res.akSeasonMail[1]);
                            global.playerMgr.onGiveMail(this._parent.plt, this._parent.id, SeDefine_1.SeMailType.SYSTEM, content, items, 0, title);
                        }
                    }
                }
                // 结算的时候需要把成就系统也结算了
                ...
                if (seasonlog.pvplevel >= 16)
                    this._baseInfo.duowang_count += 1;
                if (seasonlog.pvpscore >= 4000)
                    this._baseInfo.high_duowang_count += 1;
                if (seasonlog.fengwangCount == 1)
                    this._baseInfo.fengwang_count += 1;
                this._baseInfo.pvp_level = newLevel;
                this._baseInfo.pvp_score = newScore;
                this._baseInfo.pvp_star = 1;
                this._baseInfo.log_1v1 = new SePlayerDef_1.PvpLogInfo();
                this._baseInfo.log_2v2 = new SePlayerDef_1.PvpLogInfo();
                this._baseInfo.log_peak = new SePlayerDef_1.PvpLogInfo();
                ...
                // 结算的时候需要把巅峰系统也结算了
                global.chartMgr.onGiveSeasonReward(this.seasonid, SeDefine_1.SeChartType.SCT_PEAK_SCORE, this._parent.id, this.peak_score);
                if (TeConfig_1.configInst.get('globalMgr.url-all') && global.resMgr.getConfig("peak_cross_server") == 'true') {
                    global.globalChartMgr.onGiveSeasonCrossReward(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PEAK_SCORE, this._parent.id, this.peak_score);
                }
                if (TeConfig_1.configInst.get('globalCsMgr.url')) {
                    global.globalChartMgr.onGiveSeasonCrossReward(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_GLORY_SCORE, this._parent.id, this.glory_score);
                }
                // 清理一下玩家身上的赛季重置后清除道具
            ...
            // 结算的时候需要把关卡竞速也结算了
            if (this.level_speed) {
                global.chartMgr.onGiveSeasonReward(this.seasonid, SeDefine_1.SeChartType.SCT_PUTONG_LEVEL_SPEED, this._parent.id, this.level_speed[0]);
                global.chartMgr.onGiveSeasonReward(this.seasonid, SeDefine_1.SeChartType.SCT_KUNNAN_LEVEL_SPEED, this._parent.id, this.level_speed[1]);
                if (TeConfig_1.configInst.get('globalCsMgr.url')) {
                    global.globalChartMgr.onGiveSeasonCrossReward(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_DIYU_LEVEL_SPEED, this._parent.id, this.level_speed[2]);
                }
            }
            this._baseInfo.level_speed = [];
            this._baseInfo.level_speed_level = "";
            // 结算的时候需要把诸侯伐董也结算了,并刷新新对手
            if (this._baseInfo.pve_pk_rank < SeDefine_1.PVE_PK_INIT_VALUE && TeConfig_1.configInst.get('globalMgr.url-all')) {
                global.globalChartMgr.onGiveSeasonCrossReward(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE, this._parent.id, this._baseInfo.pve_pk_rank);
            }
            this._baseInfo.pve_pk_rank = SeDefine_1.PVE_PK_INIT_VALUE;
    ...
    checkPeakDayReward() {
        ...
        if (TeTool_1.TeDate.daydiff(this.peak_day_etime, Date.now()) > 0) { // && this.peak_day_etime < last_reward_time
            //注意置到当天得凌晨, 以前得处理完
        ...
        if (time > 0) {
            if (this.peak_score > SeDefine_1.SCORE_INIT_VALUE) {
                global.chartMgr.onGiveDayReward(this.seasonid, SeDefine_1.SeChartType.SCT_PEAK_SCORE, this._parent.id, this.peak_score, this.peak_day_etime, time);
            }
            else {
    ...
    */
    checkPvePkDayReward() {
        ...
        let time = 0;
        //最后一下发奖励时间和当前时间的日期差大于1并且最后一下发奖励时间小于昨天的发奖励时间
        if (TeTool_1.TeDate.daydiff(this.pve_pk_day_etime, Date.now()) > 0) { // && this.peak_day_etime < last_reward_time
            //注意置到当天得凌晨, 以前得处理完
            time = new Date().setHours(0, 0, 0, 0);
        }
        if (time > 0) {
            if (this.pve_pk_rank < SeDefine_1.PVE_PK_INIT_VALUE) {
                global.globalChartMgr.onGiveDayCrossReward(this.seasonid, SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE, this._parent.id, this.pve_pk_rank, this.pve_pk_day_etime, time);
            }
            else {
                //将奖励时间重置, 没有必要等到榜单服务器的返回
    ...
    giveSeasonReward(reward) {
        if (reward && reward.items) {
            ...
            if (content) {
                ...
                if (strList.length == 3) {
                    ...
                    global.playerMgr.onGiveMail(this._parent.plt, this._parent.id, SeDefine_1.SeMailType.SYSTEM, message, items, 0, title);
                }
            }
            // this._parent.addItems(items, "seasonend");
        }
        ...
        //清空
        this.peak_score = SeDefine_1.SCORE_INIT_VALUE;
        this._baseInfo.win_peak_count = 0;
        this._baseInfo.lose_peak_count = 0;
        this.saveInfo(['win_peak_count', 'lose_peak_count', 'peak_score']);
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'win_peak_count', this._baseInfo.win_peak_count);
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'lose_peak_count', this._baseInfo.lose_peak_count);
        //名将统计清理
    ...
    giveSeasonCrossReward(reward) {
        ...
        let max_player = 200;
        for (let i in chartTable) {
            if (chartTable[i].eType == interface_1.SeEnumChartTableeType.QuanQuZhengBaBang) {
     ...
     * 跨服巅峰赛季结算奖励
     */
    givePvePkSeasonCrossReward(reward) {
        if (reward && reward.items && reward.items.length > 0) {
            let items = [];
            for (let i = 0; i < reward.items.length; i++) {
                let _items = reward.items[i].split(',');
     ...
     * 关卡竞速赛赛季结算奖励
     */
    giveLevelSpeedSeasonReward(reward, chartype) {
        if (chartype == interface_1.SeEnumChartTableeType.QuanFuRongYaoJiFenBang) {
            this.glory_score = 0;
            this.glory_kill = 0;
        ...
        if (reward && reward.items && reward.items.length > 0) {
            ...
            // //发送奖励邮件
            let title = TeTool_1.LangID("4930");
            let message = TeTool_1.LangID("4931");
            switch (chartype) {
                case interface_1.SeEnumChartTableeType.PuTongGuanKaJingSuSaiBang:
                    message += TeTool_1.LangID("4932");
                    break;
                case interface_1.SeEnumChartTableeType.QuanFuPuTongGuanKaJingSuSaiBang:
                    message += TeTool_1.LangID("4932");
                    break;
                case interface_1.SeEnumChartTableeType.KunNanGuanKaJingSuSaiBang:
                    message += TeTool_1.LangID("4933");
                    break;
                case interface_1.SeEnumChartTableeType.QuanFuKunNanGuanKaJingSuSaiBang:
                    message += TeTool_1.LangID("4933");
                    break;
                case interface_1.SeEnumChartTableeType.DiYuGuanKaJingSuSaiBang:
                    message += TeTool_1.LangID("4934");
                    break;
                case interface_1.SeEnumChartTableeType.QuanFuDiYuGuanKaJingSuSaiBang:
                    message += TeTool_1.LangID("4934");
                    break;
                case interface_1.SeEnumChartTableeType.QuanFuRongYaoJiFenBang:
                    title = TeTool_1.LangID("5149");
                    message += TeTool_1.LangID("5150");
                    break;
            }
            message += TeTool_1.LangID("4935") + reward.grade + TeTool_1.LangID("4936");
            global.playerMgr.onGiveMail(this._parent.plt, this._parent.id, SeDefine_1.SeMailType.SYSTEM, message, items, 0, title);
        }
    }
    /**
     * 赏金赛数据刷新
     */
    refreshShangjinCount() {
        this._baseInfo.win_shangjin_count = 0;
        this._baseInfo.lose_shangjin_count = 0;
        this._baseInfo.shangjin_score = SeDefine_1.SCORE_INIT_VALUE;
     ...
     * 巅峰每日结算奖励
     */
    giveDayReward(reward, pass_time) {
     ...
     * 诸侯伐董每日结算奖励
     */
    givePvePkDayReward(reward, pass_time) {
        for (let i in reward) {
            let _reward = reward[i].items;
            if (_reward) {
                let items = [];
                ...
                if (content) {
                    let strList = content.split('{#}');
                    let message = strList[0] + TeTool_1.TeDate.Date_Format(new Date(reward[i].rtime), "yyyy-MM-dd") + strList[1] + reward[i].grade + strList[2];
                    if (strList.length == 3) {
                        global.playerMgr.onGiveMail(this._parent.plt, this._parent.id, SeDefine_1.SeMailType.SYSTEM, message, items, 0, title);
                    }
                }
    ...
    add_pve_pk_record(season_id, time, opp_name, type, is_win, rank, rank_change) {
        ...
        if (this._parent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this._parent.linkid, 'pve_pk_record', this._baseInfo.pve_pk_record);
    }
}
exports.SePvpMgr = SePvpMgr;
// 这里保存静态的赛季结算公式
class SeasonFunc {
    ...
    static regist_fun(name, cb) {
        this.func_map.set(name, cb);
    }
}
SeasonFunc.func_map = new TeTool_1.Map();
function func_elo_scale(elo) {
    var base = 500;
    var a = 1;
    var b = 500;
    var out = base;
...
SeasonFunc.regist_fun('level1', function (new_elo, old_elo, level) {
    var elo_base = 1500, a = 50, k = 0.75;
    return Math.max(Math.floor((new_elo - elo_base) / a * k + level * (1 - k)), 1);
});
SeasonFunc.regist_fun('award1', function (new_level, old_level, level) {
    return Math.max(Math.floor(new_level + old_level), 1) * 20;
});
//# sourceMappingURL=SePvpMgr.js.map