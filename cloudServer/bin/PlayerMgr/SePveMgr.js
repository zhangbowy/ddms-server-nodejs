...
class SePveMgr {
    ...
    _can_fight(levelId, times) {
        let pkLevelInfoRes = SeResMgr_1.SeResMgr.inst.LevelInfoRes.getRes(levelId);
        if (!pkLevelInfoRes) {
            return false;
        }
    ...
    notice_to_player_start(succ, levelId, times) {
    ...
    finishfight(levelId, times, hps, time, damage) {
        ...
        if (win)
            giveAward = true;
        if (pkLevelInfoRes.iProperty & interface_1.SeEnumLevelInfoiProperty.ShiBaiZhengChangDiaoLuo) {
            giveAward = true;
        }
        if (giveAward) {
            // 发送奖励
        ...
        // 如果奖励存在 那么 发给玩家道具
        if (awards.length > 0)
            this.parent.addItems(awards, 'pveawards');
        if (win) {
            // 记录一下通关难度
        ...
        // 记录一下地图的完成次数
        if (!this.baseInfo.pve_maps[levelId])
            this.baseInfo.pve_maps[levelId] = { count: 0, win: 0, damage: 0 };
        this.baseInfo.pve_maps[levelId].count += times;
        if (win)
            this.baseInfo.pve_maps[levelId].win += times;
        ...
        if (pkLevelInfoRes.iRankType & interface_1.SeEnumLevelInfoiRankType.ShangHaiPaiXing) {
            global.chartMgr.addPlayerLevelChart(this.parent.pvpMgr.seasonid, interface_1.SeEnumChartTableeType.ShangHaiPaiXing, {
    ...
    checkSeason() {
        ...
        for (let key in this.baseInfo.pve_maps) {
            let pkLevelInfoRes = SeResMgr_1.SeResMgr.inst.LevelInfoRes.getRes(key);
            if (!pkLevelInfoRes || (pkLevelInfoRes.iProperty & interface_1.SeEnumLevelInfoiProperty.SaiJiZhongZhi)) {
    ...
    notice_to_player_finish(succ, win, levelId, awards = [], mapstatics) {
        global.netMgr.sendData({
            cmd: 'pveaward',
            succ: succ,
            win: win,
...
exports.SePveMgr = SePveMgr;
//# sourceMappingURL=SePveMgr.js.map