...
class SeBufferMgr {
    ...
    del_buffer(bfID) {
        if (!this.bufferdatas.hasOwnProperty(bfID))
            return;
    ...
    add_buffer(bfID) {
        ...
        if (!rInfo) {
            rInfo = {
                id: bfID,
                optime: curr,
                ftime: (pkRes.eType == interface_1.SeEnumTownBuffereType.YueKa || pkRes.eType == interface_1.SeEnumTownBuffereType.JingShiYueKa || pkRes.eType == interface_1.SeEnumTownBuffereType.ZhuangBeiYueKa) ? TeTool_1.TeDate.ToDate0(curr) - 1 + pkRes.iLastHour * 3600 * 1000 : pkRes.iLastHour * 3600 * 1000 + curr,
                value: 0,
                nodel: pkRes.eType == interface_1.SeEnumTownBuffereType.YueKa || pkRes.eType == interface_1.SeEnumTownBuffereType.JingShiYueKa || pkRes.eType == interface_1.SeEnumTownBuffereType.ZhuangBeiYueKa
            };
        }
        else {
            ...
            if (pkRes.iSum == 1) {
                if (rInfo.ftime > curr) {
                    ...
                    rInfo.ftime = TeTool_1.TeDate.ToDate0(curr) - 1 + pkRes.iLastHour * 3600 * 1000;
                }
            }
            else {
        ...
        if (pkRes.eType == interface_1.SeEnumTownBuffereType.YueKa) {
            if (pkRes.akGifts) {
                for (let j = 0; j < pkRes.akGifts.length; j++) {
                    this.m_pkParent.addItem(pkRes.akGifts[j].split(',')[0], parseInt(pkRes.akGifts[j].split(',')[1]), SePlayerDef_1.DeAddDelItemReason.yueka);
                }
            }
            this.check_monthvip_buff();
            this.m_pkParent.getShopInfo();
            this.m_pkParent.sendAnnouncement(interface_1.SeEnumnoticetexteType.GouMaiYueKa, { charname: this.m_pkParent.name }, this.m_pkParent);
        }
        else if (pkRes.eType == interface_1.SeEnumTownBuffereType.JingShiYueKa || pkRes.eType == interface_1.SeEnumTownBuffereType.ZhuangBeiYueKa) {
            if (pkRes.akGifts) {
                for (let j = 0; j < pkRes.akGifts.length; j++) {
                    this.m_pkParent.addItem(pkRes.akGifts[j].split(',')[0], parseInt(pkRes.akGifts[j].split(',')[1]), SePlayerDef_1.DeAddDelItemReason.yueka);
                }
        ...
        for (var key in buffs) {
            ...
            if (rbuff) {
                if (rbuff.ftime < curr && !rbuff.nodel) {
                    ...
                    if (pkRes && pkRes.eType == interface_1.SeEnumTownBuffereType.YueKa) {
                        b_shop_notice = true;
                    }
    ...
    checkSeasonBuff() {
        ...
        for (var bid in pkRes) {
            if ((pkRes[bid].iProperty & interface_1.SeEnumTownBufferiProperty.SaiJiQingChu) == interface_1.SeEnumTownBufferiProperty.SaiJiQingChu) {
    ...
    get_monthvip_buff() {
        ...
        if (this.m_pkParent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.m_pkParent.linkid, 'bufferunitems', this.bufferunitems);
        ...
        if (this.bufferundays > 0) {
            this.bufferundays = 0;
            if (this.m_pkParent.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.m_pkParent.linkid, 'bufferundays', this.bufferundays);
        }
    }
    get_monthvip_buff_v2(bfid) {
        ...
        if (this.m_pkParent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.m_pkParent.linkid, 'bufferunitems_v2', this.m_pkParent.baseInfo.bufferunitems_v2);
        for (let __tid in items) {
        ...
        if (this.m_pkParent.baseInfo.bufferundays_v2[index] > 0) {
            ...
            if (this.m_pkParent.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.m_pkParent.linkid, 'bufferundays_v2', this.m_pkParent.baseInfo.bufferundays_v2);
        }
    }
    /**
     * 检查月卡buff的效果
    ...
    check_monthvip_buff() {
        ...
        if (TeTool_1.TeDate.daydiff(rBuff.optime, curr) < 0) {
            // 操作过了的这天就pass
            return;
        }
        var pkRes = global.resMgr.TownBufferRes.getRes(bfid);
        ...
        // 这里给玩家发送一个邮件
        var diff = Math.max(TeTool_1.TeDate.daydiff(rBuff.optime, curr) + 1, 1);
    ...
    check_monthvip_buff_v2(bfid) {
        ...
        for (let k = 0; k < bfid_all.length; k++) {
            ...
            var index = -1;
            for (var i = 0; i < bfid_index_all.length; i++) {
                if (bfid_index_all[i] == bfid_all[k])
                    index = i;
            }
            if (index == -1)
                continue;
    ...
    */
    ...
    getMaxChenHaoBuff() {
        ...
        for (var key in this.bufferdatas) {
            ...
            if (buffRes.eType == interface_1.SeEnumTownBuffereType.ChenHao) {
    ...
    send_to_player(optbff) {
        if (this.m_pkParent.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.m_pkParent.linkid, 'buffup', this.bufferdatas, optbff);
    }
}
exports.SeBufferMgr = SeBufferMgr;
//# sourceMappingURL=SeBuffMgr.js.map