...
class SeItemMgr {
    ...
    check_item_whenload() {
        ...
        if (VerUpgrade_1.UpgradeLib.itemUpgrade(this.baseInfo.items, this.baseInfo.version, global.resMgr.getConfig('userver'))) {
   ...
   */
    getItems(typeid) {
        ...
        for (let i = 0; i < this.baseInfo.items.length; i++) {
            let rkBagItem = this.baseInfo.items[i];
            ...
            if (rkBagItem.end && rkBagItem.end < curr)
                continue;
            if (rkBagItem.kItemID != typeid)
                continue;
            outs.push(rkBagItem);
        }
        if (change)
    ...
    addItem(typeid, num, reason, subReason) {
        ...
        if (pkTownItemRes.kEndTime && Date.parse(pkTownItemRes.kEndTime) <= Date.now())
            return false;
        //任务统计
        if (global.resMgr.isTaskItem(typeid)) {
            this.m_pkParent.taskAction(SeDefine_1.TaskAction.AddItem, typeid, pkTownItemRes.eTypeA, num);
        }
        // 这里增加一个道具日志
        // 这里增加一个道具日志
        if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.ManZuTiaoJianTiHuanWuPin) == interface_1.SeEnumTownItemiProperty.ManZuTiaoJianTiHuanWuPin) {
        ...
        if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.HuoQuShiShiYongBingTongZhi) == interface_1.SeEnumTownItemiProperty.HuoQuShiShiYongBingTongZhi) {
            //如果皮肤已拥有，返回钻石
            if (pkTownItemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.YingXiongPiFu) {
        ...
        else if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.HuoQuJiShiYong) == interface_1.SeEnumTownItemiProperty.HuoQuJiShiYong) {
            //如果皮肤已拥有，返回钻石
            if (pkTownItemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.YingXiongPiFu) {
                if (this.hadSkin(pkTownItemRes.kValueA)) {
                    this.addItem('W001', Number(pkTownItemRes.kValueC) * num, 'repeat_' + typeid, subReason);
        ...
        let curr = Date.now();
        if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.BuKeDieJia) != interface_1.SeEnumTownItemiProperty.BuKeDieJia) {
            ...
            for (let i = 0; i < this.baseInfo.items.length; i++) {
                ...
                if (r_item.kItemID != typeid)
                    continue;
                if (r_item.end && !pkTownItemRes.iDurationTime)
                    continue;
                if (!r_item.end && pkTownItemRes.iDurationTime)
                    continue;
                bfind = true;
                let notice = true;
                if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.FuGaiTianJia) == interface_1.SeEnumTownItemiProperty.FuGaiTianJia) {
                    // 覆盖添加的时候替换掉之前的次数
                    r_item.iPileCount = num;
                    if (pkTownItemRes.iDurationTime)
                        r_item.end = pkTownItemRes.iDurationTime * 60 * 1000 + curr;
                    log_type = 'radd';
                }
                // 如果道具已经到最大值了，那么添加后实际数量不变化，自动吃掉多余的数量，主要是有些功能道具实现冗余激活的
                    ...
                    //如果同时有赛季清除特性，取最小值
                    if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.SaiJiJieSuanHouZhongZhi) == interface_1.SeEnumTownItemiProperty.SaiJiJieSuanHouZhongZhi
                        || (pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.SaiJiJieSuanQianZhongZhi) == interface_1.SeEnumTownItemiProperty.SaiJiJieSuanQianZhongZhi) {
                    ...
                    if (pkTownItemRes.iMaxPileCount) {
                        ...
                        r_item.iPileCount = r_item.iPileCount + num;
                }
        ...
        // 不可叠加道具
        if (!bfind) {
            let newItem = new SePlayerDef_1.SeItem();
        ...
        this.m_pkParent.saveBaseInfo('items');
        if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.HuoQuJiShiYong) != interface_1.SeEnumTownItemiProperty.HuoQuJiShiYong) {
    ...
    getItemCount(typeid) {
        let curr = Date.now();
        var rkItems = this.getItems(typeid);
        let tot = 0;
    ...
    delItem(typeid, num, reason, subReason) {
        if (num == 0)
            return true;
        ...
        if (pkTownItemRes && global.resMgr.isTaskItem(typeid)) {
            this.m_pkParent.taskAction(SeDefine_1.TaskAction.DelItem, typeid, pkTownItemRes.eTypeA, num);
        }
        return true;
    }
    /**
        ...
        for (let i = 0; i < awards.length; i++) {
            ...
            if (r && r.uid == uid) {
                if (r.award) {
                    ...
                    r.award = true;
                    this.addItem(itemID, itemNum, SePlayerDef_1.DeAddDelItemReason.get_share_award);
    ...
    _can_send_award_(uid, type) {
        if ((!type || type == SeDefine_1.SeShareType.yaoshi) && this.m_pkParent.dailyInfo.wx_share_send.indexOf(uid) >= 0) {
            return false;
        }
        else if (type == SeDefine_1.SeShareType.zhanqi && this.m_pkParent.dailyInfo.wx_share_send_zhanqi.indexOf(uid) >= 0) {
            return false;
        }
        else if (type == SeDefine_1.SeShareType.other && this.m_pkParent.dailyInfo.wx_share_send_other.indexOf(uid) >= 0) {
            return false;
        }
        return true;
    }
     ...
     * 分析每日分享送的钥匙数据
     */
    add_daily_share_links_(mailInfo) {
        // 这里做一个时间判断
        if (TeTool_1.TeDate.isdiffday(mailInfo.cttime))
            return;
        try {
            let d_info = JSON.parse(mailInfo.message);
            if (!d_info.share_type || d_info.share_type == SeDefine_1.SeShareType.yaoshi) {
                d_info.award = false;
            ...
            else if (d_info.share_type == SeDefine_1.SeShareType.zhanqi) {
                d_info.award = true;
                if (!this.m_pkParent.dailyInfo.wx_shared_get_zhanqi)
                    this.m_pkParent.dailyInfo.wx_shared_get_zhanqi = [];
                ...
                this.m_pkParent.addItems(mailInfo.items, SePlayerDef_1.DeAddDelItemReason.get_share_zhanqi_award);
                this.m_pkParent.dailyInfo.wx_shared_get_zhanqi.push(d_info);
                this.m_pkParent.updateDailyInfo();
            }
            else if (d_info.share_type == SeDefine_1.SeShareType.other) {
                d_info.award = true;
                if (!this.m_pkParent.dailyInfo.wx_shared_get_other)
                    this.m_pkParent.dailyInfo.wx_shared_get_other = [];
                ...
                this.m_pkParent.taskAction(SeDefine_1.TaskAction.ShareLink);
     ...
     */
    share_keys_award(share_uid, share_time, share_type) {
        if (!share_uid || !share_time)
            return;
        let uid = parseInt(share_uid);
        if (TeTool_1.TeDate.isdiffday(share_time)) {
            // 说明连接过期了
            this._share_keys_award('timeout');
        }
        else {
            if (!share_type || share_type == SeDefine_1.SeShareType.yaoshi) {
                ...
                if (this._can_send_award_(uid, share_type)) {
                    ...
                    if (this.m_pkParent.dailyInfo.wx_share_send.length < inviteCount) {
                        // 自己获得一个奖励
                        this.addItem(itemID, itemNum, SePlayerDef_1.DeAddDelItemReason.share_keys_award);
                    ...
                    // 发送一个奖励消息给别人
                    if (uid != this.m_pkParent.id)
                        global.playerMgr.onGiveMail(this.m_pkParent.plt, uid, SeDefine_1.SeMailType.ShareBack, JSON.stringify({ uid: this.m_pkParent.id, icon: this.m_pkParent.icon, avatar: this.m_pkParent.avatar, share_type: share_type }), [{ kItemID: item[0], iPileCount: parseInt(item[1]) }]);
            ...
            else if (share_type == SeDefine_1.SeShareType.zhanqi) {
                if (this._can_send_award_(uid, share_type)) {
                    // 发送一个奖励消息给别人
                    let itemID = 'DH014';
                    let itemNum = 1;
                    if (uid != this.m_pkParent.id)
                        global.playerMgr.onGiveMail(this.m_pkParent.plt, uid, SeDefine_1.SeMailType.ShareBack, JSON.stringify({ uid: this.m_pkParent.id, icon: this.m_pkParent.icon, avatar: this.m_pkParent.avatar, share_type: share_type }), [{ kItemID: itemID, iPileCount: itemNum }]);
                    if (!this.m_pkParent.dailyInfo.wx_share_send_zhanqi)
                        this.m_pkParent.dailyInfo.wx_share_send_zhanqi = [];
                    this.m_pkParent.dailyInfo.wx_share_send_zhanqi.push(uid);
                    this.m_pkParent.updateDailyInfo();
                }
            }
            else if (share_type == SeDefine_1.SeShareType.other) {
                if (this._can_send_award_(uid, share_type)) {
                    // 发送一个奖励消息给别人
                    if (uid != this.m_pkParent.id)
                        global.playerMgr.onGiveMail(this.m_pkParent.plt, uid, SeDefine_1.SeMailType.ShareBack, JSON.stringify({ uid: this.m_pkParent.id, icon: this.m_pkParent.icon, avatar: this.m_pkParent.avatar, share_type: share_type }), []);
                    if (!this.m_pkParent.dailyInfo.wx_share_send_other)
                        this.m_pkParent.dailyInfo.wx_share_send_other = [];
    ...
    clear_season_before_item() {
        ...
        for (let i = 0; i < this.baseInfo.items.length; i++) {
            ...
            if (!pkTownItemRes)
                continue;
            if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.SaiJiJieSuanQianZhongZhi) == interface_1.SeEnumTownItemiProperty.SaiJiJieSuanQianZhongZhi) {
                this.baseInfo.items.splice(i, 1);
                i--;
                change = true;
            }
     ...
     */
    checkItemWhenChart(typeId) {
        // 
        let charId = SeResMgr_1.SeResMgr.inst.getItem2CharType(typeId);
        if (charId == null)
            return;
        // 判断榜单是否存在
        let pkChartRes = SeResMgr_1.SeResMgr.inst.chartTable.getRes(charId);
        if (!pkChartRes)
            return;
        // 检查榜单的时间
        if (pkChartRes.kStartDate && Date.now() < Date.parse(pkChartRes.kStartDate))
            return;
        if (pkChartRes.kEndDate && Date.now() > Date.parse(pkChartRes.kEndDate))
            return;
        let itemcount = this.getItemCount(typeId);
        global.chartMgr.addPlayerLevelChart(this.m_pkParent.pvpMgr.seasonid, pkChartRes.eType, {
...
exports.SeItemMgr = SeItemMgr;
//# sourceMappingURL=SeItemMgr.js.map