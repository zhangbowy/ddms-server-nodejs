...
exports.SePlayer = void 0;
const TableRes = require("../Res/interface");
const interface_1 = require("../Res/interface");
const SeItemMgr_1 = require("./SeItemMgr");
const SePlayerDef_1 = require("./SePlayerDef");
const SeHeroCardMgr_1 = require("./SeHeroCardMgr");
const SeDefine_1 = require("../SeDefine");
const SeShopMgr_1 = require("./SeShopMgr");
const SePvpMgr_1 = require("./SePvpMgr");
const SeEquipMgr_1 = require("./SeEquipMgr");
...
const SeBuffMgr_1 = require("./SeBuffMgr");
const SeFriendMgr_1 = require("./SeFriendMgr");
const VerUpgrade_1 = require("./VerUpgrade");
const interface_2 = require("../Res/interface");
const crypto_1 = require("crypto");
const SeMailMgr_1 = require("./SeMailMgr");
const SePveMgr_1 = require("./SePveMgr");
const SeSignMgr_1 = require("./SeSignMgr");
const SeCallBackMgr_1 = require("./SeCallBackMgr");
const SePveNewMgr_1 = require("./SePveNewMgr");
const SeGuildMgr_1 = require("./SeGuildMgr");
var http = require('http');
class SePlayer {
    constructor(charid, account, plt) {
        this.loadflag = 0;
        this.needdelete = 0;
        this.bInitComplete = false;
        this.baseInfo = new SePlayerDef_1.SeBaseCharInfo();
        this.m_pkItemMgr = new SeItemMgr_1.SeItemMgr(this);
        this.m_pkHeroCardMgr = new SeHeroCardMgr_1.SeHeroCardMgr(this);
        this.m_shangjinHeroCardMgr = new SeHeroCardMgr_1.SeHeroCardMgr(this);
        this.m_pkShopMgr = new SeShopMgr_1.SeShopMgr(this);
        this.pvpMgr = new SePvpMgr_1.SePvpMgr(this);
        this.m_taskMgr = new SeTaskMgr_1.SeTaskMgr(this);
        this.m_buffMgr = new SeBuffMgr_1.SeBufferMgr(this);
        this.friendMgr = new SeFriendMgr_1.SeFriendMgr(this);
        this.pveMgr = new SePveMgr_1.SePveMgr(this);
        this.pveNewMgr = new SePveNewMgr_1.SePveNewMgr(this);
        this.m_equipMgr = new SeEquipMgr_1.SeEquipMgr(this);
        this.m_signMgr = new SeSignMgr_1.SeSignMgr(this);
        this.m_guildMgr = new SeGuildMgr_1.SeGuildMgr(this);
        this.m_callbackMgr = new SeCallBackMgr_1.SeCallBackMgr(this);
        this.mailMgr = new SeMailMgr_1.SeMailMgrV2(this);
        this.plt = 'sdw';
        this.slince_mode = false;
        this._real_level = 1;
        this.room_state_info = {
            roomkey: '',
            roomleavetype: '',
            roomtype: '' // 暂时没有使用
        };
        this._rankCache = {};
        this.last_recharge_check_time = 0;
        this._totalRechargeCount = 0;
        this._iFirstPay = 0;
        this.check_name_code = 4;
        this._extInfo = null;
        this.amount_score_max = { 30: 2000, 98: 3500, 198: 7000, 648: 20500 };
        this.plt = TeConfig_1.configInst.get("plt");
        this.account = account;
        this.baseInfo = new SePlayerDef_1.SeBaseCharInfo({ charid: charid });
        this.baseInfo.charid = charid;
        this.m_kDBLoader = TePlayerLoader_1.mysqlLoaderInst.getLoader(charid);
        var nationType = Math.round(Math.random() * 2);
        this.baseInfo.nation = SeDefine_1.SeNationTypeNames[nationType];
        this.loadReload();
    ...
    get baseInfoDB() {
        return this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead.baseInfo);
    }
    get offline_mailInfoDB() {
        return this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead.mailInfo);
    }
    get pvpDb() {
        return this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead.pvpInfo);
    }
    get rechargeDB() {
        return this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead.rechargeInfo);
    }
    get taskDb() {
        return this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead.taskinfo);
    }
    get shopDb() {
        return this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead.shopinfo);
    }
    setRaceID(arg0) {
        this.pvpMgr.setRaceID(arg0);
    }
    _loaderComplete(bSucc) {
        if (!bSucc) {
            // 一般不会出现
        }
        ...
        if (bSucc) {
            ...
            this.operateLoading(SeDefine_1.SeCharLoadFlag.dbcomplete);
        }
    }
    // 刷新数据
    freshkey(key) {
    ...
    get dailyInfo() {
        if (TeTool_1.TeDate.isdiffday(this.baseInfo.dailyInfo.freshTime)) {
            this.baseInfo.dailyInfo = new SePlayerDef_1.SeDailyInfo();
    ...
    updateDailyInfo() {
        ...
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'dailyinfo', this.dailyInfo);
    }
    get flipLottery() {
        //翻牌子时间
        ...
        if (activity && Date.parse(activity.kStartTime) < Date.now() && Date.parse(activity.kEndTime) > Date.now()) {
            ...
            for (var key in shopMalls) {
                ...
                if (!flipLottery) {
                    ...
                    //判断是否需要刷新
                    if ((flipLottery.refreshDay == 1 && TeTool_1.TeDate.isdiffday(flipLottery.freshTime)) || (flipLottery.refreshDay == 2 && TeTool_1.TeDate.daydiff(flipLottery.freshTime) >= 2) || (flipLottery.refreshDay == 3 && TeTool_1.TeDate.daydiff(flipLottery.freshTime) >= 3) || (flipLottery.refreshDay == 4 && TeTool_1.TeDate.daydiff(flipLottery.freshTime) >= 4)) {
                        //刷新时间在活动开始之前的
    ...
    //刷新翻牌子
    _refreshFlipLottery(flipLottery, dailyFresh = false, firstFresh, startTime) {
        var upNeedHeros = [];
        ...
        if (dailyFresh) {
            ...
            if (flipLottery.refreshDay == 2) {
                if (TeTool_1.TeDate.daydiff(Date.parse(startTime)) % 2 == 0) {
                ...
                if (TeTool_1.TeDate.daydiff(Date.parse(startTime)) % 3 == 0) {
                    flipLottery.freshTime = new Date().setHours(0, 0, 0, 0);
                }
                //第二天和第三天登录刷新时间设置成第一天
                else {
                    let curDate = new Date().setHours(0, 0, 0, 0);
                    flipLottery.freshTime = curDate - 24 * 60 * 60 * 1000 * (TeTool_1.TeDate.daydiff(Date.parse(startTime)) % 3);
                }
                ...
                if (TeTool_1.TeDate.daydiff(Date.parse(startTime)) % 4 == 0) {
                    flipLottery.freshTime = new Date().setHours(0, 0, 0, 0);
                }
                //刷新时间设置成第一天
                else {
                    let curDate = new Date().setHours(0, 0, 0, 0);
                    flipLottery.freshTime = curDate - 24 * 60 * 60 * 1000 * (TeTool_1.TeDate.daydiff(Date.parse(startTime)) % 4);
                }
    ...
    getDBValue(type) {
        ...
        //     case 
        // }
        var r = this.m_kDBLoader.getDB(SeDefine_1.SeDBInfoHead[type]);
        if (r) {
            vvv = r.value;
        }
        return vvv;
    ...
    get loadComplete() { return !(this.state == SeDefine_1.CharState.loading || this.state == SeDefine_1.CharState.nochar || this.state == SeDefine_1.CharState.offline); }
    hasCharLoadFlag(flag) { return (this.loadflag & flag) == flag; }
    /*
     基础信息加载  老版本是使用一个字符串的，现在要改个版本，使用hash数组的模式
     */
    loadbaseInfo(bSuccess) {
        // if (this.checkDB != num) return;
        var dbValue = this.baseInfoDB.value;
        // global.logMgr.log(dbValue);
        if (bSuccess && dbValue['level']) {
            this.baseInfo = new SePlayerDef_1.SeBaseCharInfo(this.baseInfoDB.value);
            ...
            // 检查一下是否被锁了
            var bLock = false;
            if (this.baseInfo.lockType == SePlayerDef_1.LockType.TimeLock) {
                if (this.baseInfo.lockTime > Date.now()) {
                    bLock = true;
                }
            }
            else if (this.baseInfo.lockType == SePlayerDef_1.LockType.NormalLock) {
                bLock = true;
            }
            if (bLock) {
                this.state = SeDefine_1.CharState.lock;
                ...
                this.operateLoading(SeDefine_1.SeCharLoadFlag.baseinfo);
                return true;
            }
        }
        else {
            this.state = SeDefine_1.CharState.nochar;
    ...
    loadPvpInfo(bSuccess) {
        if (bSuccess) {
            this.pvpMgr.initDbInfo(this.pvpDb.value);
        }
        this.operateLoading(SeDefine_1.SeCharLoadFlag.pvpInfo);
        return bSuccess;
    }
    loadTaskInfo(bSuccess) {
        if (bSuccess) {
            this.m_taskMgr.initDb(this.taskDb);
        }
        this.operateLoading(SeDefine_1.SeCharLoadFlag.taskinfo);
        return bSuccess;
    }
    loadShopInfo(bSuccess) {
        if (bSuccess) {
            this.m_pkShopMgr.initDb(this.shopDb.value);
        }
        this.operateLoading(SeDefine_1.SeCharLoadFlag.shopinfo);
        return bSuccess;
    }
    //--------------------------
    leaveGame() {
        ...
        //若是第二天，只保留当日0点
        if (TeTool_1.TeDate.isdiffday(this.baseInfo.loginTime)) {
        ...
        if (this.loadComplete)
            this.m_kDBLoader.saveInfo();
        this.setState(SeDefine_1.CharState.offline);
    }
    get id() { return this.baseInfo.charid; }
    set name(name) { this.baseInfo.charname = TeTool_1.formationName(name); }
    get name() { return this.baseInfo.charname; }
    set servertime(servertime) { this.baseInfo.servertime = servertime; }
    get servertime() { return this.baseInfo.servertime; }
    get timezome() { return this.baseInfo.timezone; }
    set nation(nation) { this.baseInfo.nation = nation; }
    ...
    check_level() {
        ...
        if (!pkBattleLevel || !pkBattleLevel.iNeedScore || pkBattleLevel.iNeedScore > this.baseInfo.score)
            return;
        ...
        if (this.baseInfo.level >= 9) {
            this.sendAnnouncement(interface_1.SeEnumnoticetexteType.ShengJi, { playerlevel: this.baseInfo.level }, this);
        }
        this.check_level();
    }
    get score() { return this.baseInfo.score || 0; }
    get realScore() {
        var rscore = 0;
    ...
    set score(score) {
        if (typeof score != 'number' || this.score == Infinity || this.score == null)
            return;
        if (this.baseInfo.score == score)
            return;
        // 这里要做好日志监控，防止出现问题
        if (score < this.score) {
        ...
        if (this.loadComplete) {
            if (this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'score', this.score, this.level);
            this.baseInfoDB.msave([{ k: 'score', v: this.score }, { k: 'level', v: this.level }]);
            if (this.level >= 3)
                global.chartMgr.addPlayerLevelChart(this.pvpMgr.seasonid, SeDefine_1.SeChartType.SCT_SCORE, { seasonid: this.pvpMgr.seasonid, id: this.id, name: this.name, score: this.realScore, icon: this.icon, avatar: this.avatar, igroup: this.pvpMgr.groupId, is_vip: this.baseInfo.is_vip, vip_level: this.baseInfo.vip_level });
        }
        return;
    }
    get avatar() {
    ...
    set icon(icon) {
        ...
        if (this.baseInfo.icon != icon) {
            ...
            if (this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'icon', this.icon);
        }
        // if (systemicon) {
     ...
     */
    decMoney(num, reason, ...subParam) {
        if (!num)
            return false;
        // if (typeof num != 'number') return false;
        var addList = ['chapteraward', 'recharge', 'zuanshiitem', 'callbackadd'];
        if (num < 0 && addList.indexOf(reason) < 0)
            return false;
        if (this.baseInfo.money < num)
            return false;
        var before = this.baseInfo.money;
        this.baseInfo.money = before - num;
        if (num > 0) {
            this.taskAction(SeDefine_1.TaskAction.UseMoney, num);
        }
        // 这里要做好日志监控，防止出现问题
        ...
        if (this.loadComplete) {
            if (this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'money', this.money);
        }
        return true;
    }
    /**
    ...
    useGold(num, noEnoughBuy = true) {
        ...
        if (this.baseInfo.gold < num) {
            ...
            if (!this.decMoney(needMoney, "buyGold"))
                return;
            this.gold = 0;
        }
        else {
    ...
    _changeGold(gold, maxLimit, reason, notice = true) {
        ...
        if (gold < this.baseInfo.gold) {
            this.taskAction(SeDefine_1.TaskAction.UseGold, this.baseInfo.gold - gold);
        ...
        if (this.loadComplete) {
            if (this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'gold', this.gold, null, notice ? reason : null);
        }
        return;
    }
    set gold(number) { this._changeGold(number, true); }
    // 可以不受上限控制的增加金币
    set forcegold(number) { this._changeGold(number, false); }
    //带上原因的更改金币
    changeGold(number, reason, notice = true) { this._changeGold(number, true, reason, notice); }
    onCreateChar(shareUid) {
    ...
    _sdw_pay() {
        ...
        str += '|' + crypto_1.createHash('md5').update(('' + this.loginInfo.appid + this.plt + this.loginInfo.channel + this.id)).digest("hex");
        return str;
    }
    get real_level() {
    ...
    sum_tot_level() {
        ...
        for (let i = 0; i < this.baseInfo.heros.length; i++) {
            let r_hero = this.baseInfo.heros[i];
            if (!r_hero) {
                continue;
            }
            let t_hero = {
            ...
            for (let j = t_hero.iLevel; j < mxLevel; j++) {
                ...
                if (!cost)
                    break;
                if (t_hero.iCount < cost.card && cost.card >= 0)
                    break;
                if (cost.gold > curr_gold)
                    break;
        ...
        for (let i = curr_level; i < global.resMgr.MaxLvl; i++) {
            ...
            if (!pkBattleLevel || !pkBattleLevel.iNeedScore || pkBattleLevel.iNeedScore > curr_score)
                break;
    ...
    operateLoading(flag) {
        ...
        if (this.hasCharLoadFlag(SeDefine_1.SeCharLoadFlag.complete)) {
            this.slince_mode = true;
            // 再设置一次icon的检查
            this.icon = this.baseInfo.realicon;
            this.setState(SeDefine_1.CharState.loadcomplete);
            ...
            try {
                ...
                this.m_pkHeroCardMgr.checkFormationsWhenLoad();
                this.m_pkItemMgr.check_item_whenload();
                this.score = VerUpgrade_1.UpgradeLib.ScoreUpgrade(this.baseInfo.level, this.baseInfo.score, this.baseInfo.version, global.resMgr.getConfig('userver'));
            }
            catch (e) {
                console.error(e);
            }
            ...
            this.m_taskMgr.doAction_slince(SeDefine_1.TaskAction.Recharge, this._totalRechargeCount);
            this.m_taskMgr.doAction_slince(SeDefine_1.TaskAction.FirstPay, this._iFirstPay);
            ...
            this.taskAction(SeDefine_1.TaskAction.Login);
            ...
            global.chartMgr.queryranks(this.pvpMgr.seasonid, this.id, SeDefine_1.SeChartType.SCT_PVP_SCORE);
            global.globalChartMgr.queryranks(this.pvpMgr.seasonid, this.id, SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE);
            this.slince_mode = false;
            ...
            //老玩家回归活动
            if (this.level >= 3 && TeTool_1.TeDate.daydiff(this.baseInfo.lastLoginTime) >= 14 && TeTool_1.TeDate.daydiff(this.baseInfo.lastLoginTime) <= 365) {
                this.oldPlayerRefresh();
            }
            var currVer = global.resMgr.getConfig('userver');
            //sdw平台需要上报胜场数
            if (TeConfig_1.configInst.get('plt') == 'sdw') {
            ...
            if (this.baseInfo.checkPveStar != check_pve_num && this.pvpMgr.groupId) {
                ...
                if (star_count > 0) {
                    ...
                    global.chartMgr.addPlayerLevelChart(this.pvpMgr.seasonid, TableRes.SeEnumChartTableeType.pveXingShuBang, {
                        id: this.id,
                        name: this.name,
                        score: score,
                        icon: this.icon,
            ...
            if (this.baseInfo.checkPvePk != check_pve_pk) {
                this.pvpMgr.set_pve_pk_rank(SeDefine_1.PVE_PK_INIT_VALUE);
                global.globalChartMgr.queryranks(this.pvpMgr.seasonid, this.id, SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE);
    ...
    check_toy_reward() {
        ...
        if (global.playerMgr.toy_info[toy_id] && global.playerMgr.toy_info[toy_id].is_complete && !this.baseInfo.toy_camp.get_reward) {
            ...
            if (rewards_all) {
                ...
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM, TeTool_1.LangID("5705"), items, 0, TeTool_1.LangID("5706"));
    ...
    random_toy_camp() {
        //堆玩具玩法获得阵营
        if (this.baseInfo.toy_camp && this.baseInfo.toy_camp.id != -1)
            return;
        let toy_activity = global.resMgr.getActivityByType(TableRes.SeEnumActivityeType.ZhenYingDuiKang);
        if (Date.parse(toy_activity.kStartTime) > Date.now() || Date.parse(toy_activity.kEndTime) < Date.now())
            return;
        let id = -1;
        //先判断是否在前100名
        let index = SeDefine_1.toy_totcharge_100.indexOf(this.id);
        if (index != -1) {
    ...
    oldPlayerRefresh() {
        ...
        this.taskAction(SeDefine_1.TaskAction.HuiGuiRecharge, 5000);
        //回归道具令牌清除
    ...
    checkVIPWeekmail() {
        if (this.baseInfo.is_vip && this.baseInfo.vip_level > 0) {
            ...
            if (vip_res && vip_res.akRewardWeek) {
                if (TeTool_1.TeDate.isdiffweek(this.baseInfo.vip_weekmail_time)) {
    ...
    refreshActivity() {
        ...
        if (!toy_activity || Date.now() < Date.parse(toy_activity.kStartTime) || Date.now() > Date.parse(toy_activity.kShowEndTime)) {
            this.baseInfo.toy_camp = new SePlayerDef_1.SeToyInfo();
        }
        //神秘商店代币返还
        ...
        if (store_activity && Date.now() > Date.parse(store_activity.kEndTime)) {
            ...
            if (count > 0 && this.delItem('W033', count, 'return_money')) {
                ...
                let title = '神秘代币返还';
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM, content, items, 0, title);
            }
        ...
        if (treasure_activity && Date.now() > Date.parse(treasure_activity.kEndTime)) {
            ...
            this.baseInfo.new_year_treasure = new SePlayerDef_1.SeNewYearTreasure();
        }
    ...
    _use_login_mails(PVP_MAILS, AUTO_MAILS, ThreeUrlMails, Peak_DailyReward_Mails, LevelSpeedSeasonRewards, GuildItem, PvePkSeasonRewards, CallBackMsgs) {
        try {
            ...
            if (CallBackMsgs && CallBackMsgs.length > 0)
                this.m_callbackMgr.onReciveMsg(CallBackMsgs);
        }
        catch (e) {
        }
    }
    ...
    onChangeFormation(plan, formation) {
        // 先判断一下阵型是否是全空的.
        // var allEmpty = true;
    ...
    onChangePVEPKFormation(plan, formation) {
    ...
    onChangeFormationName(plan, name) {
        if (TeTool_1.getByteLen(name) > 10) {
    ...
    onChangeShangJinFormation(shangjinFormation) {
    ...
    onChangeShangJinLord(lordId) {
        if (this.baseInfo.shangjin_lords.indexOf(lordId) == -1)
            return;
    ...
    addNewBox(boxType, level, num) {
        this.pvpMgr.addNewBox(boxType, level, num);
    }
    // 这里只负责处理使用道具的逻辑，扣除道具检查道具数量在 useItem 函数中
    _useItem(typeid, num, param1 = null, param2 = null) {
        var useDel = true;
        //[兵粮卡:0] [金元宝:1] [经验丸:2] [技能书:3] [领主技:4] [补给箱:5] [英雄经验丸:6]
        var pkTownItemRes = global.resMgr.TownItemRes.getRes(typeid);
        if (!pkTownItemRes) {
            return false;
        }
        switch (pkTownItemRes.eTypeA) {
            case interface_1.SeEnumTownItemeTypeA.JinYuanBao: {
                //[金元宝:1]
            ...
            case interface_1.SeEnumTownItemeTypeA.YingXiongKa: {
                this.m_pkHeroCardMgr.addHeroCard(pkTownItemRes.kValueA, num);
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.TiYanYingXiongKa: {
                let level = parseInt(pkTownItemRes.kValueC || '1');
                let extra = {};
                let str = pkTownItemRes.kValueB;
                if (str) {
                    ...
                    if (jLists.length == 2) {
                        switch (jLists[0]) {
                            case 'rank':
                                extra['ltype'] = 'rank';
                                extra['lvalue'] = parseInt(jLists[1]);
                                break;
                            case 'date':
                                extra['ltype'] = 'date';
                                extra['lvalue'] = Date.now() + (parseInt(jLists[1]) * 86400000);
                                break;
                            case 'match':
                                extra['ltype'] = 'match';
                                extra['lvalue'] = parseInt(jLists[1]);
            ...
            case interface_1.SeEnumTownItemeTypeA.WanJiaJingYan: {
                this.score = this.score + num * parseInt(pkTownItemRes.kValueA);
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.DengJiBaoXiang: {
                ...
                if (!heroBoxTypeRes)
                    return false;
                if (pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.BaoXiangTianJia) {
            ...
            case interface_1.SeEnumTownItemeTypeA.DuanWeiBaoXiang: {
                let boxType = parseInt(pkTownItemRes.kValueB);
            ...
            case interface_1.SeEnumTownItemeTypeA.HeZi: {
                let addItmes = [];
                ...
                if (addItmes.length > 0)
                    this.addItems(addItmes, "unzip pack");
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.LiBao:
            case interface_1.SeEnumTownItemeTypeA.BaoXiang:
            case interface_1.SeEnumTownItemeTypeA.SaiJiKaBao: {
                this.m_pkShopMgr.openChanceBox(pkTownItemRes.kId, pkTownItemRes.kValueA, num);
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.ZuanShi: {
            ...
            case interface_1.SeEnumTownItemeTypeA.BuffKa: {
                if (pkTownItemRes.kValueA)
                    this.m_buffMgr.add_buffer(pkTownItemRes.kValueA);
                if (pkTownItemRes.kValueB)
                    this.m_buffMgr.add_buffer(pkTownItemRes.kValueB);
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.ZhenFaSuiPian: {
            ...
            case interface_1.SeEnumTownItemeTypeA.TouXiangKuang: {
                // 使用头像框道具 就是给自己装备上当前的头像框
                this.baseInfo.iconid = pkTownItemRes.kId;
                this.saveBaseInfo('iconid');
                if (this.bInitComplete)
                    global.netMgr.sendCharMiscUpdate(this.linkid, "iconid", this.baseInfo.iconid);
            ...
            case interface_1.SeEnumTownItemeTypeA.PiPeiJiFen: {
                var addnum = num * parseInt(pkTownItemRes.kValueA);
                if (!addnum) {
                    addnum = 0;
                }
            ...
            case interface_1.SeEnumTownItemeTypeA.DuanWeiXingXing: {
                var addnum = num * parseInt(pkTownItemRes.kValueA);
                if (!addnum) {
                    addnum = 0;
                }
            ...
            case interface_1.SeEnumTownItemeTypeA.ZengJiaFengWangYinJi: {
                var addnum = num * parseInt(pkTownItemRes.kValueA);
                if (!addnum) {
                    addnum = 0;
                }
            ...
            case interface_1.SeEnumTownItemeTypeA.ZhuChengJingYan: {
                var addnum = num * parseInt(pkTownItemRes.kValueA);
                if (!addnum) {
                    addnum = 0;
                }
            ...
            case interface_1.SeEnumTownItemeTypeA.LeiJiChongZhi: {
                let rInfo = {
                ...
                if (this.bInitComplete) {
                    ...
                    // 检查一下重置任务
                    this.m_taskMgr.doAction(SeDefine_1.TaskAction.Recharge, this._totalRechargeCount);
                    this.m_taskMgr.doAction(SeDefine_1.TaskAction.FirstPay, this._iFirstPay);
                }
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.ZhuChengPiFu: {
                ...
                if (mcityid) {
                    ...
                    if (str) {
                        ...
                        if (jLists.length == 2) {
                            switch (jLists[0]) {
                                case 'day':
                                    this.m_pkHeroCardMgr.addbosss(mcityid, Date.now() + (parseInt(jLists[1]) * 86400000), parseInt(pkTownItemRes.kValueC), typeid);
                                    break;
                            }
                        }
                    }
            ...
            case interface_1.SeEnumTownItemeTypeA.HuiZhang: {
                ...
                if (badgeid) {
                    ...
                    if (str) {
                        ...
                        if (jLists.length == 2) {
                            switch (jLists[0]) {
                                case 'day':
                                    this.addMedal(badgeid, time + (parseInt(jLists[1]) * 86400000));
                                    break;
                            }
                        }
                    }
            ...
            case interface_1.SeEnumTownItemeTypeA.YingXiongPiFu: {
                ...
                if (mskinid) {
                    ...
                    if (str) {
                        ...
                        if (jLists.length == 2) {
                            switch (jLists[0]) {
                                case 'day':
                                    this.m_pkHeroCardMgr.addskin(mskinid, Date.now() + (parseInt(jLists[1]) * 86400000), parseInt(pkTownItemRes.kValueC), typeid);
                                    break;
                            }
                        }
                    }
            ...
            case interface_1.SeEnumTownItemeTypeA.BuHuiShouJiangChi: {
            ...
            case interface_1.SeEnumTownItemeTypeA.QuanYingXiongLiBao: {
                // 使用了一个全英雄道具，这个很危险，使用的时候都打印一下
                console.log(`use item QuanYingXiongLiBao [${this.id}] ID [${pkTownItemRes.kId}:${num}] Time [${TeTool_1.TeDate.DateToStr(Date.now())}]`);
                let list = [];
                var allHeros = global.resMgr.UnitRes.getAllRes();
                for (var key in allHeros) {
                    var rkUnitRes = allHeros[key];
            ...
            case interface_1.SeEnumTownItemeTypeA.YinDaoZuoBiDaoJu: {
            ...
            case interface_1.SeEnumTownItemeTypeA.ZhuangBei: {
            ...
            case interface_1.SeEnumTownItemeTypeA.ShangPinJieSuo: {
                pkTownItemRes;
                this.m_pkShopMgr.update_shop_limit_time(pkTownItemRes.kValueA, parseInt(pkTownItemRes.kValueB || '1'));
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.ZhuGongKa: {
                this.addLord(pkTownItemRes.kValueA);
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.QinMiDuBuff: {
                ...
                if (this.baseInfo.hero_skin_record.indexOf(skin_id) == -1) {
                    ...
                    if (this.bInitComplete)
                        global.netMgr.sendCharMiscUpdate(this.linkid, 'hero_skin_record', this.baseInfo.hero_skin_record);
                }
                break;
            }
            case interface_1.SeEnumTownItemeTypeA.ShenMiShangDianZiXuan: {
            ...
            case interface_1.SeEnumTownItemeTypeA.ZengJiaLianSheng: {
                    ...
                    if (pkTownItemRes.kValueB == '1011' && chartTable[i].eType == TableRes.SeEnumChartTableeType.PaiWeiLianSheng) {
                        ...
                        if (chartTable[i].iOpenGrade <= this.pvp_level) {
                            if (this.pvpMgr.pvp_top_wincount >= 2) {
                                global.chartMgr.addPlayerLevelChart(this.pvpMgr.seasonid, SeDefine_1.SeChartType.SCT_1V1_WIN, { seasonid: this.pvpMgr.seasonid, id: this.id, name: this.name, score: this.pvpMgr.pvp_top_wincount, icon: this.icon, avatar: this.avatar, igroup: this.pvpMgr.groupId, is_vip: this.baseInfo.is_vip, vip_level: this.baseInfo.vip_level });
                            }
                        }
                    }
                    if (pkTownItemRes.kValueB == '1022' && chartTable[i].eType == TableRes.SeEnumChartTableeType.ZuDuiLianSheng) {
                        ...
                        if (chartTable[i].iOpenGrade <= this.pvp_level) {
                            if (this.pvpMgr.pvp_top_win2v2count >= 2) {
                                global.chartMgr.addPlayerLevelChart(this.pvpMgr.seasonid, SeDefine_1.SeChartType.SCT_2V2_WIN, { seasonid: this.pvpMgr.seasonid, id: this.id, name: this.name, score: this.pvpMgr.pvp_top_win2v2count, icon: this.icon, avatar: this.avatar, igroup: this.pvpMgr.groupId, is_vip: this.baseInfo.is_vip, vip_level: this.baseInfo.vip_level });
                            }
                        }
                    }
                }
        ...
        if ((pkTownItemRes.iProperty & interface_1.SeEnumTownItemiProperty.TuiSongDiSanFang) == interface_1.SeEnumTownItemiProperty.TuiSongDiSanFang) {
    ...
    addCommonNotice(type, title, notice, show = false) {
        global.netMgr.sendCommonNotice(this.linkid, type, title, notice, show);
    }
    useItem(typeid, num, param1 = null, param2 = null) {
        if (num <= 0) {
            return false;
        }
        ...
        this.m_pkItemMgr.delItem(typeid, num, SePlayerDef_1.DeAddDelItemReason.useitem);
        return true;
    }
    itemCount(typeid) {
    ...
    addHeroCardBatch(arr, notice = true, boxID) {
    ...
    cheatMsg(params) {
        switch (params[0]) {
            ...
            case '-gold': {
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM, '想通过作弊拿钱，做梦吧', [{ kItemID: params[1], iPileCount: parseInt(params[2]) || 1 }]);
                break;
            }
            case '-getitem': {
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM, '你通过作弊拿到的道具,可耻', [{ kItemID: params[1], iPileCount: parseInt(params[2]) || 1 }]);
                break;
            }
            case '-equip_opr': {
            ...
            case '-onekeycheat': {
                ...
                for (var key in allHeros) {
                    var rkUnitRes = allHeros[key];
                    this.m_pkHeroCardMgr.addHeroCard(rkUnitRes.kID);
                    var card = this.m_pkHeroCardMgr.getHeroCard(rkUnitRes.kID);
                    //       card.iSkillA = 10;
                    //       card.iSkillB = 10;
                }
                this.saveBaseInfo();
            ...
            case "-delitem": {
                this.m_pkItemMgr.delItem(params[1], parseInt(params[2]), SePlayerDef_1.DeAddDelItemReason.cheatdel);
                break;
            }
            // case '-query': {
            //     // this.onQueryBlock();
            ...
            case '-boxtest': {
                ...
                if (pkItemRes && pkItemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.DengJiBaoXiang) {
            ...
            case '-tehuibuy': {
                ...
                if (this.bInitComplete)
                    global.netMgr.sendCharMiscUpdate(this.linkid, 'rechargestate', amoutInfo);
                break;
            }
            case '-addbuff': {
                var kid = params[1];
            ...
            case '-addmail': {
                ...
                for (let i = 0; i < count; i++) {
                    if ([SeDefine_1.SeMailType.FriendKey, SeDefine_1.SeMailType.PvpResult].indexOf(mailType) < 0 && mailType <= SeDefine_1.SeMailType.Chart) {
            ...
            case '-useAllMail': {
                for (var key in this.baseInfo.mails) {
                    var rkMail = this.baseInfo.mails[key];
            ...
            case '-linkclick': {
                ...
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.ShareBack, JSON.stringify({ uid: uid, icon: this.icon, avatar: this.avatar }), [{ kItemID: item[0], iPileCount: parseInt(item[1]) }]);
                break;
            }
            case '-testegg': {
                let ret = [];
            ...
            case '-changePvePkRank': {
                global.globalChartMgr.queryinfo(this.pvpMgr.seasonid, this.id, Number(params[1]), SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE);
                break;
            }
            case '-useHeroSkin': {
                let skin_id = params[1];
                let count = parseInt(params[2]);
                if (!this.baseInfo.hero_skin_use[skin_id])
                    this.baseInfo.hero_skin_use[skin_id] = 0;
                this.baseInfo.hero_skin_use[skin_id] = count;
                this.saveBaseInfo('hero_skin_use');
                if (this.bInitComplete)
                    global.netMgr.sendCharMiscUpdate(this.linkid, 'hero_skin_use', this.baseInfo.hero_skin_use);
                break;
            }
        }
    }
    getHistoryChart(charType, iCurrentPage, length) {
        if (charType == SeDefine_1.SeChartType.SCT_PUTONG_LEVEL_SPEED) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_KUNNAN_LEVEL_SPEED) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_DIYU_LEVEL_SPEED) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_PUTONG_LEVEL_SPEED) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_KUNNAN_LEVEL_SPEED) {
            if (TeConfig_1.configInst.get('globalCsMgr.url')) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_DIYU_LEVEL_SPEED) {
            if (TeConfig_1.configInst.get('globalCsMgr.url')) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLORY_SCORE) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_GLORY_SCORE) {
            if (TeConfig_1.configInst.get('globalCsMgr.url')) {
    ...
    getChart(charType, iCurrentPage, length) {
        if (charType == SeDefine_1.SeChartType.SCT_GROUP_PVP_SCORE) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_PVP_SCORE) {
            if (TeConfig_1.configInst.get('globalCsMgr.url')) {
                global.globalChartMgr.getPlayerLevelChart(this.pvpMgr.seasonid, charType, iCurrentPage, length, this.id, null);
            }
        }
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_PEAK_SCORE) {
            if (TeConfig_1.configInst.get('globalCsMgr.url')) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_PEAK_SCORE) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_PUTONG_LEVEL_SPEED) {
        ...
        else if (charType == SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE) {
            if (TeConfig_1.configInst.get('globalCsMgr.url')) {
                global.globalChartMgr.getPlayerLevelChart(this.pvpMgr.seasonid, charType, iCurrentPage, length, this.id, null, this.baseInfo.toy_camp.contribution);
            }
        }
    ...
    getHisChart(charType, seasionid) {
    ...
    onGetRankRet(v) {
        for (var i = 0; i < v.length; i++) {
            ...
            v[i].type == SeDefine_1.SeChartType.SCT_PVP_SCORE && this.pvpMgr && (this.pvpMgr.top_pvp_rank = v[i].rank);
            v[i].type == SeDefine_1.SeChartType.SCT_GLOBAL_PVE_OFFLINE && this.pvpMgr && this.pvpMgr.set_pve_pk_rank(v[i].rank);
        }
    ...
    onGetCharRet(charType, startIndex, length, rankInfo, rank) {
        //多维度的排行榜对score进行取整
    ...
    onGetHistoryCharRet(charType, startIndex, length, rankInfo, rank) {
        if (startIndex == undefined) {
    ...
    onGetHisCharRet(charType, sid, rankInfo, rank) {
    ...
    guideNext(data) {
        ...
        if (data.step > this.guide) {
            if (data.step >= 8 && !this.pvpMgr.groupId && !this.pvpMgr.groupState) {
                ...
                global.chartMgr.apply_group_id(this.pvpMgr.seasonid, SeDefine_1.SeChartType.SCT_GROUP_PVP_SCORE, this.id, this.pvpMgr.groupState);
            }
     ...
     */
    get roomFormation() {
        var df = this.m_pkHeroCardMgr.defaultFomation;
        var fm = [];
    ...
    get maxTeamScore() {
        var allHero = this.m_pkHeroCardMgr.heroCards && this.m_pkHeroCardMgr.heroCards.concat();
        allHero.sort(function (a, b) {
            var aPkHero = global.resMgr.UnitRes.getRes(a.kHeroID);
            var bPkHero = global.resMgr.UnitRes.getRes(b.kHeroID);
        ...
        for (var i = 0; i < allHero.length && i < 8; i++) {
            var pkHero = global.resMgr.UnitRes.getRes(allHero[i].kHeroID);
    ...
    calculate_recharge(rInfo) {
        ...
        if ((recharge_info.iProperty & interface_2.SeEnumrechargeiProperty.BuJiRuLeiJiChongZhi) != interface_2.SeEnumrechargeiProperty.BuJiRuLeiJiChongZhi) {
            this._totalRechargeCount += rInfo.amount;
        }
        if ((recharge_info.iProperty & interface_2.SeEnumrechargeiProperty.BuJiRuShouChong) != interface_2.SeEnumrechargeiProperty.BuJiRuShouChong) {
            this._iFirstPay = 1;
        }
    }
    cact_time_from_order(order) {
        // 这里在历史上有两个版本的订单规则
    ...
    addRecharge(amoutInfo, slince = false) {
        // 通过充值的额度获取配置信息
        var pkRes;
        var realAdd = 0;
        ...
        if (pkmail && pkmail.akContent)
            pkRes = global.resMgr.RechargeRes.getRes(pkmail.akContent[0]);
        //判定限购
        ...
        for (var i = 0; i < pkRes.akItem.length; i++) {
            ...
            if (pkItemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.ZuanShi) {
                realAdd = num * plus;
            }
            else if (pkItemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.HeZi && rechargeIds.indexOf(pkItemRes.kId) >= 0) {
        ...
        for (var i = 0; i < mailitems.length;) {
            ...
            let mailType = SeDefine_1.SeMailType.Charge;
            if ((pkRes.iProperty & interface_2.SeEnumrechargeiProperty.LiJiShiYong) == interface_2.SeEnumrechargeiProperty.LiJiShiYong) {
                mailType = SeDefine_1.SeMailType.AutoUse;
            }
        ...
        if (!slince) {
            ...
            this.m_taskMgr.doAction(SeDefine_1.TaskAction.Recharge, this._totalRechargeCount);
            this.m_taskMgr.doAction(SeDefine_1.TaskAction.FirstPay, this._iFirstPay);
            this.taskAction(SeDefine_1.TaskAction.ShopBuy, amoutInfo.mailid, 1);
        }
        this.m_taskMgr.doAction(SeDefine_1.TaskAction.OnceRecharge, Math.floor(amoutInfo.amount / 100));
        if (Math.random() < 0.25) {
    ...
    check_recharge_vip() {
        if (global.resMgr.getConfig('vip_system_open') == 'false')
            return;
        //如果vip已过期则重新打开
        if (!this.baseInfo.is_vip && this.baseInfo.vip_level > 0) {
        ...
        if (global.resMgr.getConfig('vip_sendmail_open') && global.resMgr.getConfig('vip_sendmail_open') == 'true' && !this.baseInfo.send_vip_mail) {
            if (recharge_amount >= 5000) {
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.GM, global.resMgr.getConfig('vip_sendmail_content'), [], 0, global.resMgr.getConfig('vip_sendmail_title'));
    ...
    send_vip_week_email(vip_level) {
        if (vip_level > 0) {
            ...
            if (vip_res && vip_res.akRewardWeek) {
                ...
                if (items.length > 0) {
                    let content = TeTool_1.LangID(vip_res.akWeekMail[0]);
                    let title = TeTool_1.LangID(vip_res.akWeekMail[1]);
                    global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM, content, items, 0, title);
    ...
    send_vip_once_email(vip_level) {
        ...
        if (vip_res && vip_res.akRewardOnce) {
            ...
            if (items.length > 0) {
                let content = TeTool_1.LangID(vip_res.akOnceMail[0]);
                let title = TeTool_1.LangID(vip_res.akOnceMail[1]);
                global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM, content, items, 0, title);
            }
        }
    }
    ...
    hasSameAmountOrder(mid, order, sid) {
        var orders = this.rechargeDB.value;
        for (var key in orders) {
            var rk = orders[key];
            if (rk && rk.mailid == mid && rk.orderid != order && (rk.sid == sid))
                return true;
        }
        return false;
    }
    ...
    onShareOk(type) {
        if (this.dailyInfo.shareCount < SePlayerDef_1.DEFINE.SHARE_DAILY_MAX) {
            let dailyShare = global.resMgr.getConfig('dailyShare');
            if (dailyShare) {
                let item = dailyShare.split(',');
                if (item.length >= 2) {
                    this.addItem(item[0], parseInt(item[1]), SePlayerDef_1.DeAddDelItemReason.share);
                }
            }
        ...
        this.taskAction(SeDefine_1.TaskAction.ShareText);
    }
    onbindPhoneNumOk(phoneNum) {
        if (this.baseInfo.phoneNum) {
            return;
        }
        this.addItem("W001", 100, SePlayerDef_1.DeAddDelItemReason.bindPhoneNum);
        this.addItem("W002", 2000, SePlayerDef_1.DeAddDelItemReason.bindPhoneNum);
        this.baseInfo.phoneNum = phoneNum;
        this.saveBaseInfo("phoneNum");
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'phoneNum', this.baseInfo.phoneNum);
    }
    onAdWatchOk(itemId) {
        ...
        if (now > (this.baseInfo.adWatchTime || 0)) {
            this.addItem(itemId, 1, SePlayerDef_1.DeAddDelItemReason.adwatch);
            //根据次数获取等待时间
            let etime = 0;
            ...
            if (this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'adWatchTime', this.baseInfo.adWatchTime);
            this.taskAction(SeDefine_1.TaskAction.AdAward, 1);
        }
    }
    onAdWatchArrayOk(index, itemId) {
        let lottery_str = global.resMgr.getConfig("lottery_cool_down");
        if (!lottery_str) {
            return;
        }
        ...
        if (now > (this.baseInfo.adWatchTime_array[index] || 0)) {
            ...
            if (this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'adWatchTime_array', this.baseInfo.adWatchTime_array);
            this.taskAction(SeDefine_1.TaskAction.AdAward, 1);
        }
    }
    //进入小游戏
    enterSmallGame(appid) {
        //平台限制
        ...
        this.updateDailyInfo();
        this.addItem('W001', 10, SePlayerDef_1.DeAddDelItemReason.esmallgame);
        ...
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'enterSmallGameAward', this.baseInfo.enterSmallGameAward);
    }
    //完成添加小程序
    completeSmallProgram() {
    ...
    addSmallProgram() {
        ...
        this.addItem('W001', 30, SePlayerDef_1.DeAddDelItemReason.smallprogram);
        global.logMgr.smallProgram(this, true, true);
    }
    //完成关注公众号
    completeFollowPublic() {
    ...
    fromFollowPublic() {
        ...
        this.updateDailyInfo();
        this.addItem('W003', 20, SePlayerDef_1.DeAddDelItemReason.smallprogram);
        // global.logMgr.fromFollowPublicCount(this, true, true);
    }
    //浮窗进入奖励
    fromScene(scene) {
        this.m_taskMgr.doAction_slince(SeDefine_1.TaskAction.FromScene, scene);
        // global.logMgr.fromFollowPublicCount(this, true, true);
    }
    /**
    ...
    //-------------------------------------------------//
    recive_room_info(type, joinkey, info, room_type) {
        switch (type) {
            case 'd_join':
                this.room_state_info.roomkey = joinkey;
                this.room_state_info.roomleavetype = SeDefine_1.NumType.N_lv_room;
                break;
            case 'join':
                this.room_state_info.roomkey = joinkey;
                this.room_state_info.roomleavetype = SeDefine_1.NumType.N_2v2_leave_room;
                break;
            case 'd_leave':
                this.room_state_info.roomkey = '';
                this.room_state_info.roomleavetype = '';
            ...
            case 'leave':
                ...
                break;
            case "startmatch":
                this.setState(SeDefine_1.CharState.matching);
                break;
            case "1v2_startmatch":
                this.setState(SeDefine_1.CharState.matching);
                break;
    ...
    cancell_match() {
        if (this.state == SeDefine_1.CharState.matching || this.state == SeDefine_1.CharState.inrace) {
            this.setState(SeDefine_1.CharState.loadcomplete);
    ...
    changeName(name) {
        if (global.resMgr.getConfig('closename') == 'true') {
            global.netMgr.sendCheckName(this.linkid, SeDefine_1.NameCheckCode.HasUsed, this.name);
            return;
        }
        if (this.name == name) {
            return;
    ...
    changePersonSign(personSign) {
        if (this.dailyInfo.person_signCount >= 15 || global.resMgr.getConfig('change_person_sign') != 'true') {
            global.netMgr.sendCheckPersonSign(this.linkid, SeDefine_1.NameCheckCode.CountLimit, this.baseInfo.personSign);
            return;
        }
    ...
    //数美屏蔽字
    _process_forbid_name_check(uid, name, cb) {
        if (name == '') {
            cb(name, true);
            return;
        }
        //hago不判定
        if (TeConfig_1.configInst.get('plt').indexOf('hago') == 0) {
            cb(name, true);
            return;
        }
        var post_data = {
    ...
    _changeName(name, result = true) {
        if (!result) {
            global.netMgr.sendCheckName(this.linkid, SeDefine_1.NameCheckCode.HasUsed, name);
            return;
        }
        if (this.name == name) {
            global.netMgr.sendCheckName(this.linkid, SeDefine_1.NameCheckCode.HasUsed, this.name);
            ...
            TePlayerLoader_1.mysqlLoaderInst.checkname(name, this.changeNameDBLoaded.bind(this, name));
        }
    }
    _forceRename(name, result = true) {
        //检查通过不改名
        if (result)
            return;
        else {
    ...
    _changePersonSign(personSign, result = true) {
        if (!result) {
            global.netMgr.sendCheckPersonSign(this.linkid, SeDefine_1.NameCheckCode.HasUsed, this.baseInfo.personSign);
            return;
        }
        this.baseInfo.personSign = personSign;
        this.saveBaseInfo(['personSign']);
        global.netMgr.sendCheckPersonSign(this.linkid, SeDefine_1.NameCheckCode.Ok, this.baseInfo.personSign);
     ...
     * @param db
     */
    changeNameDBLoaded(newName, err, db) {
        if (this.state == SeDefine_1.CharState.offline)
            return;
        if (db && db.value) {
            global.netMgr.sendCheckName(this.linkid, SeDefine_1.NameCheckCode.HasUsed, this.name);
        }
            ...
            TePlayerLoader_1.mysqlLoaderInst.renamename(this.name, newName, this.id);
            ...
            global.netMgr.sendCheckName(this.linkid, SeDefine_1.NameCheckCode.Ok, this.name);
        }
    }
    force_check_name() {
        if (this.baseInfo.forcename != this.check_name_code) {
            // 这里还要询问第三方借口
            if (TeConfig_1.configInst.get('plt').indexOf('hago') == -1) {
    ...
    //--------------任务成就相关---------------//
    taskAction(action, ...arg) {
        this.m_taskMgr.doAction(action, ...arg);
    }
    getTaskAward(kId, extinfo) {
        this.m_taskMgr.getReward(kId, extinfo);
    }
    battleAction(action, ...arg) {
        this.taskAction(SeDefine_1.TaskAction.Battle, action, ...arg);
        if (action == SeDefine_1.BattleAction.UseCard) {
            this.pvpMgr.useHero(arg[0]);
        }
        else if (action == SeDefine_1.BattleAction.UseSkin) {
            let skin_id = arg[0];
            let count = 1;
    ...
    clientTaskAction(action, ...arg) {
        var arr = [SeDefine_1.TaskAction.LookVideo, SeDefine_1.TaskAction.SayHello, SeDefine_1.TaskAction.Foucs, SeDefine_1.TaskAction.ShareVideo];
     ...
     * 只对赛季结算开放
     */
    checkSeasonChengJiu_start(pre_season_id) {
    ...
    //-----------------xxx-------------------//
    buyShopMall(mallId, marketId, count) {
    ...
    checkSystemMail() {
        ...
        for (let h_i = 0; h_i < hashs.length; h_i++) {
            ...
            if (!hash)
                continue;
            for (let key in hash) {
                let pkSysMail = hash[key];
                if (this.baseInfo.sysmailCeche && this.baseInfo.sysmailCeche[pkSysMail.kID] == pkSysMail.kStartTime)
                    continue;
                if ((pkSysMail.iProperty & TableRes.SeEnumSystemMailiProperty.JinXianFuFei) == TableRes.SeEnumSystemMailiProperty.JinXianFuFei && this._iFirstPay == 0)
                    continue;
                if ((pkSysMail.iProperty & TableRes.SeEnumSystemMailiProperty.JinXianiOS) == TableRes.SeEnumSystemMailiProperty.JinXianiOS && this.loginInfo.device_os != 'ios')
                    continue;
                if ((pkSysMail.iProperty & TableRes.SeEnumSystemMailiProperty.JinXianAnZhuo) == TableRes.SeEnumSystemMailiProperty.JinXianAnZhuo && this.loginInfo.device_os != 'andriod')
                    continue;
                if ((pkSysMail.iProperty & TableRes.SeEnumSystemMailiProperty.JinXianPC) == TableRes.SeEnumSystemMailiProperty.JinXianPC && this.loginInfo.device_os != 'pc')
                    continue;
                ...
                if (((pkSysMail.iProperty & TableRes.SeEnumSystemMailiProperty.BuPanDingZhuCe) == TableRes.SeEnumSystemMailiProperty.BuPanDingZhuCe
                    ...
                    global.playerMgr.onGiveMail(this.plt, this.id, SeDefine_1.SeMailType.SYSTEM_NO_DEL, pkSysMail.kDesc, items, (startTime - now) / 1000, pkSysMail.kTitle, endTime);
                }
    ...
    sendAnnouncement(type, content, player) {
        if (content) {
    ...
    sendAnnouncement2(type, content, player) {
        if (content) {
            content.charname = this.name;
            let buffId = this.m_buffMgr.getMaxChenHaoBuff();
            if (buffId) {
    ...
    takeDeskAward() {
        if (this.loginInfo && !this.baseInfo.bDeskAward1) {
            this.addItem('W001', 50, SePlayerDef_1.DeAddDelItemReason.deskAward);
    ...
    getliveraces(mode) {
        if (!this.hasProperty(SeDefine_1.PlayerProperty.GM_OB)) {
            return;
        }
    ...
    getroommatch(type, value, ready, kuid, index) {
        //匹配前出阵英雄校验
        ...
        let data;
        switch (type) {
            case SeDefine_1.NumType.N_2v2_join_room:
            case SeDefine_1.NumType.N_1v1:
            case SeDefine_1.NumType.N_pve_pk:
            case SeDefine_1.NumType.N_jn_room:
            case SeDefine_1.NumType.N_ct_room:
            case SeDefine_1.NumType.N_1v2_join_room:
                {
                    data = {
                        cmd: 'roomopr',
                        type: type,
        ...
        if (type == SeDefine_1.NumType.N_1v2_join_room) {
            data.p.formation.heros_skin = this.baseInfo.heros_skin;
        }
        //如果是放置玩法，主公要改成阵容里的
        else if (type == SeDefine_1.NumType.N_pve_pk) {
            let lordId = '';
        ...
        if (TeConfig_1.configInst.get('globalMgr.url-all')) {
            switch (type) {
                case SeDefine_1.NumType.N_2v2_join_room:
                case SeDefine_1.NumType.N_2v2_ready_room:
                case SeDefine_1.NumType.N_2v2_leave_room:
                    ...
                    break;
                case SeDefine_1.NumType.N_1v2_join_room:
                case SeDefine_1.NumType.N_1v2_ready_room:
                case SeDefine_1.NumType.N_1v2_leave_room:
    ...
    loadExtInfo(f) {
        this._extInfo = f;
        this.operateLoading(SeDefine_1.SeCharLoadFlag.extinfo);
        this.friendMgr.load_plt_friend();
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'ext_info', this._extInfo);
    }
    /**
     * 发送给某个玩家QQ信息
     * @param frd
     ...
     * @param qua
     */
    send_gamebar_msg(frd, msgtype, content, qua) {
        if (this._extInfo)
            global.lsMgr.sendgamebar_msg(this.id, this._extInfo.openid, this._extInfo.openkey, frd, msgtype, content, qua);
    }
    get_buffer_value(v) {
        let o_v = this.m_buffMgr.get_buffer_value(v);
        if (v == interface_1.SeEnumTownBuffereType.XiangZiJiaSu && this.isMonthVip) {
    ...
    loadGroupId(iGroupId) {
        this.pvpMgr.setGroupId(iGroupId);
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'iGroupId', iGroupId);
    }
    get_share_award(uid) {
     ...
     * 添加主公
     */
    addLord(id, timeout = 10180180211923) {
        //永久的替代限时的
        ...
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'lords', this.baseInfo.lords);
    }
    get lord() {
    ...
    addMedal(id, timeout) {
        ...
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'medals', this.baseInfo.medals);
    }
    get curMedals() {
    ...
    setcurMedal(id, oid) {
        ...
        this.saveBaseInfo('curMedals');
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'curMedals', this.baseInfo.curMedals);
    }
    enterShangJinSai(type) {
        if (this.baseInfo.shangjinState != SePlayerDef_1.ShangJinState.NOENTER) {
            return false;
        }
        ...
        if (type == 'item') {
            if (this.useItem('W024', Number(count))) {
                this.baseInfo.shangjinState = SePlayerDef_1.ShangJinState.ENTER;
            }
        }
        else if (type == 'zuanshi') {
            //废弃钻石报名赏金赛
            return false;
            let menpiao = global.resMgr.SuperMarketRes.getRes("SJS1");
            if (this.decMoney(menpiao.iPrice * Number(count), 'buyitem', "SJS1", count)) {
                this.baseInfo.shangjinState = SePlayerDef_1.ShangJinState.ENTER;
            }
        }
        if (this.baseInfo.shangjinState == SePlayerDef_1.ShangJinState.ENTER) {
            //初始抽奖次数
            this.baseInfo.shangjinCount++;
            this.saveBaseInfo(['shangjinState', 'shangjinCount']);
            //刷新胜场数及赏金任务 阵容
            this.pvpMgr.refreshShangjinCount();
            this.m_taskMgr.baomingFresh(interface_1.SeEnumTaskiTab.ShangJinSaiShengChang);
            //通知状态
    ...
    chooseShangJinHero(type) {
        if (this.baseInfo.shangjinState != SePlayerDef_1.ShangJinState.ENTER || this.baseInfo.drawTimes <= 0) {
            return false;
        }
        //抽卡
        ...
        for (let i = 0; i < heroBoxZZYRes.iTypeCount; i++) {
            let lucky = TeTool_1.arrayRandomT(poolByType.pool, 'weight', true);
            if (lucky) {
    ...
    addDrawTimes(type, count) {
        ...
        if (type == 'item') {
            ...
            let item = global.resMgr.SuperMarketRes.getRes("SJS2");
    ...
    cancelShangJinSai(needGetReward = false) {
        if (this.baseInfo.shangjinState != SePlayerDef_1.ShangJinState.ENTER) {
            return;
        }
        if (needGetReward) {
            this.baseInfo.shangjinState = SePlayerDef_1.ShangJinState.FINISH;
        }
        else {
            this.pvpMgr.refreshShangjinCount();
            this.baseInfo.shangjinState = SePlayerDef_1.ShangJinState.NOENTER;
        }
    ...
    TuiSongDiSanFang(pkTownItemRes, num) {
        if (!pkTownItemRes)
            return;
        let data = {};
        let ctime = Date.now();
     ...
     */
    getPvpMatchInfo(perMode = false, isShangjin = false) {
        var wear_equips = [];
        this.refreshLords();
        if (isShangjin) {
    ...
    getBattleSkins() {
        ...
        for (let i = 0; i < this.baseInfo.items.length; i++) {
            ...
            if (!item || !item.kItemID)
                continue;
            let itemRes = global.resMgr.TownItemRes.getRes(item.kItemID);
            if (itemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.ZhenDiPiFu) {
    ...
    getBattleBanners() {
        ...
        for (let i = 0; i < this.baseInfo.items.length; i++) {
            ...
            if (!item || !item.kItemID)
                continue;
            let itemRes = global.resMgr.TownItemRes.getRes(item.kItemID);
            if (itemRes.eTypeA == interface_1.SeEnumTownItemeTypeA.ZhanQi) {
    ...
    refreshtlmb() {
        if (Date.now() < this.baseInfo.tlmb_finish_time || this.baseInfo.tlmb_finish_time == 0)
            return;
    ...
    //阵营捐献
    contribute(count) {
        if (this.baseInfo.toy_camp.id == -1)
            return;
        if (count > 400)
            return;
        if (this.delItem('W034', count, 'contribute')) {
            ...
            if (this.baseInfo.toy_camp.contribution >= 20) {
                global.globalChartMgr.addPlayerLevelChart(this.pvpMgr.seasonid, charttype, {
    ...
    selectHerosCompose(heroId, items, amount) {
        if (this.baseInfo.selectHeros.indexOf(heroId) == -1)
            return;
        //验证数量及道具是否作弊
        let score = 0;
        for (let i = 0; i < items.length; i++) {
        ...
        if (!this.baseInfo.selectHerosCompose[heroId])
            this.baseInfo.selectHerosCompose[heroId] = {};
    ...
    pve_pk_buy(type) {
        ...
        switch (type) {
            ...
            case 'fight':
                ...
                if (this.dailyInfo.pve_pk_fight_buy_count >= cost_arr.length - 1)
                    cost = cost_arr[cost_arr.length - 1];
                else
                    cost = cost_arr[this.dailyInfo.pve_pk_fight_buy_count];
    ...
    random_discount() {
        ...
        if (vip_res.kchance) {
            ...
            let lucky = TeTool_1.arrayRandomT(pool, 'weight');
    ...
    set new_year_treasure(value) {
        this.baseInfo.new_year_treasure = value;
        this.saveBaseInfo('new_year_treasure');
    }
    //选择大奖
    selectLuckyItem(itemId, count) {
        let new_year_treasure = this.new_year_treasure;
        //翻一次之后大奖不可变
        if (new_year_treasure.treasure_count > 0)
            return;
        new_year_treasure.lucky_id = itemId;
        new_year_treasure.lucky_count = Number(count);
        this.new_year_treasure = new_year_treasure;
        if (this.bInitComplete)
            global.netMgr.sendCharMiscUpdate(this.linkid, 'new_year_treasure', this.baseInfo.new_year_treasure);
    }
    //前往下一层
    goToNextLevel() {
        let new_year_treasure = this.new_year_treasure;
        //开到大奖才可以进入下一层
        if (new_year_treasure.level >= 15 || !new_year_treasure.lucky_get)
            return;
    ...
    //年兽秘宝单个兑换
    treasureExchangeSimple(index, notice = true) {
        let lucky_get = false; //是否开到大奖
        if (this.new_year_treasure.treasure_info[index] || this.new_year_treasure.lucky_get || !this.new_year_treasure.lucky_id)
            return;
        ...
        if (this.delItem('DH016', 1)) {
            ...
            if (notice && this.bInitComplete)
                global.netMgr.sendCharMiscUpdate(this.linkid, 'new_year_treasure', this.baseInfo.new_year_treasure);
        }
        return lucky_get;
    }
    //年兽秘宝多个兑换
    treasureExchangeBatch() {
        if (this.new_year_treasure.lucky_get)
            return;
        let index_array = [];
        let treasure = this.new_year_treasure;
        //看看有哪些要抽
        for (let i = 0; i < 25; i++) {
            if (!treasure.treasure_info[i])
                index_array.push(i);
        }
...
exports.SePlayer = SePlayer;
//# sourceMappingURL=SePlayer.js.map