...
class SeCallBackMgr {
    ...
    static makeCallBackMail(uid, type, value, time) {
        return {
            uid: uid,
            type: type,
    ...
    pushMsg(type, value, time) {
        if (this.callbackinfo.from_uid) {
            if (global.resMgr.getConfig("usenormal_give") == "true") {
                global.playerMgr.onGiveMail(this.parent.plt, this.callbackinfo.from_uid, SeDefine_1.SeMailType.CallBackMsg, JSON.stringify(SeCallBackMgr.makeCallBackMail(this.parent.id, type, value, time)));
            }
            else {
                global.playerMgr.onGiveMail_Online(this.parent.plt, this.callbackinfo.from_uid, SeDefine_1.SeMailType.CallBackMsg, JSON.stringify(SeCallBackMgr.makeCallBackMail(this.parent.id, type, value, time)));
            }
        }
    }
    onCreateChar(uid) {
    ...
    saveinfo() {
        if (this.parent.loadComplete)
            global.netMgr.sendCharMiscUpdate(this.parent.linkid, 'callbackinfo', this.callbackinfo, null, null, true);
    ...
    onReciveMsg(mails) {
        ...
        if (bchange)
            this.saveinfo();
        if (loadFriend)
            this.parent.friendMgr.loadInfo();
    }
    onAddRecharge(num, time) {
        // 不是那一期的人就直接pass掉
        if (!this.callbackinfo.from_uid)
            return;
        ...
        if (kEndTime && Date.now() > kEndTime)
            return;
        if (kStartTime && Date.now() < kStartTime)
            return;
        // 发送一份邮件过去 额度从分改成元
...
exports.SeCallBackMgr = SeCallBackMgr;
//# sourceMappingURL=SeCallBackMgr.js.map