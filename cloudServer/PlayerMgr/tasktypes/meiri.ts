...
function refresh(player: SePlayer, db: ReHash<any>, taskres: { [taskid: string]: SeResTaskEx }, taskinfo: { [taskid: string]: SeTaskItem }, ...args: any[]) {
    ...
    let random_pool_rates: { [iGroup: number]: number } = {};
    let random_pool_ids: { [iGroup: number]: SeResTaskEx[] } = {};
    ...
    if (!args[0]) {
        ...
        if (curr - preUpdateTime > 24 * 3600 * 1000) {
            ...
            for (let key in taskres) {
                let r = taskres[key];
                //同盟的判定是否在同盟任务列表里
                if(r.iTab == SeEnumTaskiTab.MengJunTongMengRenWu && player.dailyInfo.guild_task_id.indexOf(r.kTaskID) == -1) continue;
                ...
                if (player.m_taskMgr._checkTaskCanComplete(r) && (!r.iLevel || r.iLevel == player.level) && (!r.iPvpLv || r.iPvpLv <= player.pvp_level)) {
                    if (!random_pool_rates[r.iGroup]) random_pool_rates[r.iGroup] = 0;
                    random_pool_rates[r.iGroup] += (r.iRate || 0);

                    if (!random_pool_ids[r.iGroup]) random_pool_ids[r.iGroup] = [];
        ...
        if(del_taskId){
            ...
            for (let key in taskres) {
                let r = taskres[key];
                //只刷新同组的
                if (r.iGroup != taskres[del_taskId].iGroup || r.kTaskID == del_taskId) continue;
                // 找到可以完成的 然后适合当前段位的任务来随机
                if (player.m_taskMgr._checkTaskCanComplete(r) && (!r.iLevel || r.iLevel == player.level) && (!r.iPvpLv || r.iPvpLv <= player.pvp_level)) {
                    if (!random_pool_rates[r.iGroup]) random_pool_rates[r.iGroup] = 0;
                    random_pool_rates[r.iGroup] += (r.iRate || 0);
...
TaskModule.regist_pool(SeEnumTaskiModule.MeiRi, refresh);