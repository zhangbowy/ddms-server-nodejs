...
import { SignItem, DeAddDelItemReason, SeGuildInfo, SeItem } from "./SePlayerDef";
import { SeResMgr } from "../ResMgr/SeResMgr";
import { iApp } from "../app";
import { TeDate, getCharByte } from "../TeTool";
import { SeEnumMonthSignetype, SeEnumFestivalSignetype, SeEnumTaskiModule, SeEnumGuideiFight } from "../Res/interface";
import { SeGuildPlayer, SeMailType, TaskAction, SeEnumGuildItemReason } from "../SeDefine";
import { configInst } from "../lib/TeConfig";
import { SeItemMgr } from "./SeItemMgr";

declare var global: iApp;

export class SeGuildMgr {
    parent: SePlayer;
    ...
    public deal_guild_opr(data){
        let info = data.info;
        switch(data.type){
            case 'receive_help':
                //添加卡片
                let help_addItems: Array<SeItem> = [{kItemID: info.heroId, iPileCount: info.count}];
                global.playerMgr.onGiveMail(configInst.get('plt'), data.uid, SeMailType.SYSTEM, global.resMgr.getConfig('TM_donate_mail'), help_addItems, 0, global.resMgr.getConfig('TM_donate_title'));
                if(!this.guild_info.help_info) this.guild_info.help_info = [];
                //添加捐卡记录，最多10条
            ...
            case 'receive_exchange':
                //添加卡片
                let exchange_addItems: Array<SeItem> = [];
                ...
                global.playerMgr.onGiveMail(configInst.get('plt'), data.uid, SeMailType.SYSTEM, global.resMgr.getConfig('TM_exchange_mail'), exchange_addItems, 0, global.resMgr.getConfig('TM_exchange_title'));
                //添加换卡记录，最多10条
                if(!this.guild_info.exchange_info) this.guild_info.exchange_info = [];
            ...
            case 'help_result':
                ...
                if (this.parent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.parent.linkid, 'uphero', rkCard);
                let card_res = global.resMgr.UnitRes.getRes(rkCard.kHeroID);
                this.parent.taskAction(TaskAction.GuildHelp, data.count, null, card_res.iColour);
                ...
                break;
            case 'join_ret':
                if(!this.guild_info.apply_info) this.guild_info.apply_info = [];
                if(data.success){
                ...
                if(data.send_mail){
                    ...
                    if(data.success) message = '您已成功加入' + data.guild_name + '同盟';
                    if(data.out_time) message = '您的申请暂未通过';
                    global.playerMgr.onGiveMail(configInst.get('plt'), data.uid, SeMailType.SYSTEM, message, [], 0, global.resMgr.getConfig('TM_mail_title'));
                }
                break;
            case 'card_item_opr':
                //部分情况导致的变更发邮件    
                if(data.reason == SeEnumGuildItemReason.cancel_exchange){
                    let additems: Array<SeItem> = [];
                    ...
                    global.playerMgr.onGiveMail(configInst.get('plt'), data.uid, SeMailType.SYSTEM, global.resMgr.getConfig('TM_exchange_fail_mail'), additems, 0, global.resMgr.getConfig('TM_exchange_title'));
                }
                else{
                ...
                break;
            case 'kick':
                if(this.guild_info.guild_id != data.guild_id) return;
                this.baseInfo.guild_info = new SeGuildInfo();
                ...
                global.playerMgr.onGiveMail(configInst.get('plt'), data.uid, SeMailType.SYSTEM, message);
                break;
            case 'quit_ret':
                this.baseInfo.guild_info = new SeGuildInfo();
                this.baseInfo.guild_info.quite_time = data.quite_time;
            ...
            case 'task_ret':
                ...
                this.parent.updateDailyInfo();
                this.parent.m_taskMgr.updateModuleTask(true, SeEnumTaskiModule.MeiRi, true, '', data.task_id);
                break;
            case 'invite':
                if(!this.guild_info.invite_info) this.guild_info.invite_info = [];
                if(this.guild_info.guild_id) return;
                //重复邀请检查
        ...
        this.saveBaseInfo('guild_info');
        if (this.parent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.parent.linkid, 'guild_info', this.guild_info);
    }

    private exchange_currency = ['','W116','W117','W118','W119']
    public guild_opr(data){
        //带上自己基本信息
        data['uid'] = this.parent.id;
        // id: this.id, name: this.name, score: this.pvpMgr.pvp_top_wincount, icon: this.icon, avatar: this.avatar}
        let user_info: SeGuildPlayer = {id: this.parent.id, name: this.parent.name, icon: this.parent.icon, 
        ...
        switch(data.type){
            ...
            case 'update':
                ...
                for(let i = 0; i < data.param.length; i++){
                    if(update_List.indexOf(data.param[i]) == -1) return;
                    if(data.param[i] == 'name') check_list[0] = data.value[i];
                    if(data.param[i] == 'announcement') check_list[1] = data.value[i];
                }
            ...
            case 'chat':
                //聊天长度限制30字
                if(getCharByte(data.chat_text) > 116) return;
                // global.guildMgr.sendGuildData(data);
                //数美判定
            ...
            case 'help':
                ...
                if(!this.baseInfo.dailyInfo.guild_help_count) this.baseInfo.dailyInfo.guild_help_count = [];
                if(!this.baseInfo.dailyInfo.guild_help_count[res.iColor - 1]) this.baseInfo.dailyInfo.guild_help_count[res.iColor - 1] = 0;
                if(data.count + this.baseInfo.dailyInfo.guild_help_count[res.iColor - 1] > count_limit_day) return;
                let count_limit = global.resMgr.getConfig('TM_donate_one').split(',')[res.iColor - 1];
                if(data.count > count_limit) return;
                break;
            case 'creat':
                //没同盟才能花钻石创建
            ...
            case 'ask_exchange':
                ...
                for(let use_card of data.use_card){
                    ...
                    if(color == -1) color = res.iColor;
                    else if(color != res.iColor) return;
            ...
            case 'cancel_exchange':
                ...
                break;
            case 'quite':
                if(this.guild_info.guild_id != data.id) return;
                //退出同盟
                break;
            case 'kick':
                //盟主T人
                break;
            case 'contribute':
                if(this.parent.dailyInfo.guild_contribute + data.count > parseInt(global.resMgr.getConfig('TM_donate_limit'))) return;
                if(!this.parent.decMoney(data.count, 'contribute')) return;
                this.parent.dailyInfo.guild_contribute += data.count;
                //获得个人勋章
                this.parent.addItem('W037', data.count, 'contribute');
                this.parent.updateDailyInfo();
                this.parent.taskAction(TaskAction.GuildContribute, data.count, 'W001');
                break;
            case 'apply_general':
                if(!this.parent.decMoney(200, 'apply_general')) return;
                break;
            case 'cancel_quite':
                if(!this.guild_info.last_id || this.guild_info.last_id != data.id) return;
                break;
            case 'up_level':
                break;
            case 'ask_help':
                ...
                if(!this.guild_info.last_ask_help_time) this.guild_info.last_ask_help_time = [];
    ...
    public getGuildTaskInfo(taskIds: Array<string>){
    ...
    public refuseGuildInvite(guild_id){
        for(let i = 0; i < this.guild_info.invite_info.length; i++){
    ...
    addGuildTaskValue(task_id: string, value: number){
