import { SeHeroCard, SeBaseCharInfo, SeUpgradeType, HeroBoxColorScore } from './SePlayerDef';
import { CharState, SeItemType, SeLogicFormation, TaskAction, PlayerProperty, SeCharMailInfo, SeMailType, SeChartType } from '../SeDefine';
import * as Table from "../Res/interface";
import { iApp } from "../app";
import { SePlayer } from "./SePlayer";
import { SeResUnitEx, SeResMgr } from '../ResMgr/SeResMgr';
import { UpgradeLib } from './VerUpgrade';
import { configInst } from '../lib/TeConfig';
import { LangID } from '../TeTool';

declare var global: iApp;
export class SeHeroCardMgr {
    private m_pkParent: SePlayer;

    private _totalCardNum: number = 0;
    private _totalCardScore: number = 0;
     ...
     * 登陆的时候检查阵容信息
     */
    public checkHerosWhenLoad(): boolean {
        var change = false;
        ...
        if (UpgradeLib.heroUpgrade(this.baseInfo.heros, this.baseInfo.version, global.resMgr.getConfig('userver'))) {
            change = true;
        }
     ...
     * 登陆的时候检查阵容信息
     */
    public checkFormationsWhenLoad(): boolean {
        var change = false;
        // 检查一下玩家是否需要给他初始化一个阵容
     ...
     * 获取英雄卡
     */
    public getHeroCard(kHeroID: string): SeHeroCard {
     ...
     * 获取英雄皮肤
     */
    public getHeroSkin(kHeroID: string): string {
     ...
     * 删除英雄卡
     */
    public delHeroCard(kHeroID: string): SeHeroCard {
        for (var i = 0; i < this.heroCards.length; i++) {
            var herocard = this.heroCards[i];
            if (herocard && herocard.kHeroID == kHeroID) {
     ...
     * @param lists
     */
    public addHeroCardBatch(lists: Array<{ kid: string, type: number, num: number }>, notice: boolean = true, boxID?: string) {
        var addGold = 0;
        var addItems = 0;
        var ids = {};
        //加不进卡牌之后的补偿
        ...
        this.saveHeroCards();
        if (this.m_pkParent.bInitComplete && upCards.length > 0) global.netMgr.sendCharMiscUpdate(this.linkid, 'upheros', upCards, null, notice? '' : 'pvenewawards');
    ...
    public resetHeroLv(...arg) {
        ...
        for (var i = 0; i < this.baseInfo.heros.length; i++) {
            var rkHero = <SeHeroCard>this.baseInfo.heros[i];
            var rkHeroRes = global.resMgr.UnitRes.getRes(rkHero.kHeroID);
            if (!rkHeroRes) continue;
        ...
        this.saveHeroCards();
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'upheros', this.baseInfo.heros);
    }
     ...
     */
    public addHeroCard(kHeroID: string, num: number = 1, nosave: boolean = false): Object {
        var rkHeroRes: Table.SeResUnit = global.resMgr.UnitRes.getRes(kHeroID);
        if (!rkHeroRes) {
            return;
        }
        ...
        var rkHeroCard: SeHeroCard = this.getHeroCard(kHeroID);
        if (!rkHeroCard) {
            rkHeroCard = new SeHeroCard();
            rkHeroCard.kHeroID = kHeroID;
            rkHeroCard.iCount = num;
            this.heroCards.push(rkHeroCard);

            if (this.m_pkParent.loadComplete && !nosave) {
                if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'addhero', rkHeroCard);
                this.saveHeroCards();
            }
        }
        else {
            ...
            var mxCards: number = global.resMgr.cardMxCards(kHeroID, rkHeroCard.iLevel);
            ...
            rkHeroCard.iCount += num;
            if (this.m_pkParent.loadComplete && !nosave) {
                if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'uphero', rkHeroCard);
                this.saveHeroCards();

                //计算补偿的金币数据
                var baseGold = this.m_pkParent.gold;
                var __changeCards = [changes];
                if (changes && changes.cGolds != 0) this.m_pkParent.changeGold(baseGold + changes['cGolds'], __changeCards);
                if (changes && changes.cItems != 0) this.m_pkParent.addItem('BD01', changes['cItems'], 'uphero');
            }
        }
        ...
        this._totalCardScore += num * (HeroBoxColorScore[rkHeroRes.iColour] || 1)
        return changes;
    }
     ...
     * @param _kHeroID 武将id
     */
    public addTryHeroCard(kHeroID: string, level: number, extra: Object, nosave: boolean = false) {
        var rkHeroRes: Table.SeResUnit = global.resMgr.UnitRes.getRes(kHeroID);
        if (!rkHeroRes) {
            return;
        }
        ...
        if (this.m_pkParent.loadComplete && !nosave) {
            if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, _type, rkHeroCard);
            this.saveHeroCards();
        ...
        for (var i = 0; i < df.length; i++) {
            ...
            if (herocard && herocard.oExtra && herocard.oExtra['ltype'] && herocard.oExtra['lvalue']) {
                switch (herocard.oExtra['ltype']) {
                    case "rank":
                        if (herocard.oExtra['lvalue'] <= this.m_pkParent.pvp_level) return false;
                        break;
                    case "date":
                        if (herocard.oExtra['lvalue'] + 60 * 1000 < Date.now()) return false;  //这里多加1分钟的兼容, 防止客户端与服务器的时间不一致造成bug
                        break;
                    case "match":
                        if (herocard.oExtra['lvalue'] <= 0) return false;
                        break;
                }
            }
        }
     ...
     */
    public updateTryHeroCard(ltype: string) {
        var upheros = [];
        var df = this.defaultFomation;
        for (var i = 0; i < df.length; i++) {
            var herocard = this.getHeroCard(df[i]);
        ...
        //通知
        if (upheros.length > 0) {
            if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'upheros', upheros);
        }
    }
    ...
    public delTryHeroCard(heroIDs: string[]) {
        ...
        for (var i = 0; i < heroIDs.length; i++) {
            ...
            if (herocard && herocard.oExtra) {
                ...
                //邮件通知
                let rkHeroRes: Table.SeResUnit = global.resMgr.UnitRes.getRes(heroIDs[i]);
                let strRes = global.resMgr.getConfig("trialCardExpire");
                if (strRes) {
                    //ydq 已经改为多语言客户端翻译, 直接组装转义的语句
                    let message = rkHeroRes.kName + LangID(strRes);
                    global.playerMgr.onGiveMail(this.m_pkParent.plt, this.m_pkParent.id, SeMailType.SYSTEM, message);
                }
        ...
        if (delheros.length > 0) {
            if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'delhero', delheros);
        }
    }
     ...
     * @returns 补偿金币
     */
    public clearHeroCard() {

        for (var i = 0; i < this.heroCards.length; i++) {
            var golds = 0;
            var herocard = this.heroCards[i];
            if (herocard) {
                let mxCards: number = global.resMgr.cardMxCards(herocard.kHeroID, herocard.iLevel);
                if (herocard.iCount > mxCards) {
                    let rkHeroRes: Table.SeResUnit = global.resMgr.UnitRes.getRes(herocard.kHeroID);
                    if (rkHeroRes) {
                        ...
                        if (strRes && golds > 0) {
                            //ydq 已经改为多语言客户端翻译, 直接组装转义的语句
                            let message = rkHeroRes.kName + LangID(strRes) + " " + (beforeNum - mxCards) + LangID("353") /**卡牌 */ + " " + golds + LangID("352") /**金币 */;
                            global.playerMgr.onGiveMail(this.m_pkParent.plt, this.m_pkParent.id, SeMailType.SYSTEM, message, [{ kItemID: "W002", iPileCount: golds }]);
                        }
                    }
                }
            }
    ...
    private _get_hero_color(heroID: string | SeHeroCard) {
        ...
        if (pkRes) return pkRes.iColour;
        return 0;
    }

    public upgradeCard(cardID: string) {
        var rkCard = this.getHeroCard(cardID);
        if (!rkCard) return false;
        ...
        this.m_pkParent.taskAction(TaskAction.HeroUp, rkCard.kHeroID, rkCard.iLevel);
        ...
        if (this.m_pkParent.loadComplete) {
            if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'uphero', rkCard);
        }

        var pkRes = global.resMgr.UnitRes.getRes(rkCard.kHeroID);
        if (!pkRes) return true;
    ...
    set defaultPlan(plan: number) {
        ...
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'f_plan', plan);
    }
     ...
     * @param timeout 
     */
    public addbosss(id: string, timeout: number, convert_zuanshi: number, reason: string) {
        //已有永久的直接返回钻石
        ...
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'bosss', this.baseInfo.bosss, null, reason);
    }
    ...
    public checkskin() {
        ...
        if (flush) {
            ...
            if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'heros_skin', this.baseInfo.heros_skin);
        }
     ...
     * @param timeout 
     */
    public addskin(id: string, timeout: number, convert_gold: number, reason: string) {
        ...
        if (skinitem) skinitem.time = timeout;
        else skins.push({ kid: id, time: timeout });

        this.m_pkParent.saveBaseInfo('heros_skin');
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'heros_skin_id', skinRes.kHeroId, this.baseInfo.heros_skin[skinRes.kHeroId], reason);
    }

    public setCurskin(id: string, heroid: string) {
        ...
        this.m_pkParent.saveBaseInfo('heros_skin');
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'heros_skin_id', heroid, this.baseInfo.heros_skin[heroid]);
    }
    ...
    set bossFormation(bossId: string) {
        ...
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'bossFormation', this.bossFormation);
    }
    ...
    set campboss(campbossId: string) {
        ...
        if (this.m_pkParent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.linkid, 'battlecampboss', this.campboss);
    }
    ...
    get matchBossFormation() {
        ...
        var fm1: SeLogicFormation = { kHeroID: kID, iLevel: (cardInfo && cardInfo.iLevel) || 1 };
    ...
    public getPlanFormation(plan: number) {
    ...
    public getPlanFormationLen(plan: number) {
        if (this.formation.length <= plan) {
            return 0;
        }
     ...
     * 设置阵容
     */
    public setPlanFormation(plan: number, formation: Array<string>) {
        // this.m_pkParent.clearCampainSelectCard();
        plan = plan || 0;
        formation = formation || [];
        ...
        for (var oneKey in this.heroCards) {
            var oneHeroCard: SeHeroCard = this.heroCards[oneKey];
            if (!oneHeroCard) {
                continue;
            }
     ...
     * 设置带pve属性放置玩法阵容
     */
    public setPlanPVEPKFormation(plan: number, formation: {}) {
        plan = plan || 0;
        let lordId = '';
        ...
        //如果是防守阵容的话通知榜单服修改
        if(plan == 1){
            let chart_res = global.resMgr.getChartTableByType(SeChartType.SCT_GLOBAL_PVE_OFFLINE);
     ...
     * 设置赏金赛阵容
     */
    public setShangjinFormation(shangjinFormation: Array<string>) {
        ...
        if (!this.m_pkParent.hasProperty(PlayerProperty.GM_COMPETIOR)) {
    ...
    check_card_num_limit(kHeroID: string, heroBoxZZYid: string): number{
    ...
    public _total_level_(formation?): number {
