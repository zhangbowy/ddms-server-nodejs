...
import { SeEquip } from "./SePlayerDef";
import { iApp } from "../app";
import { SeResLootPool, SeEnumChartTableeType, SeEnumLevelInfoiRankType, SeEnumLevelInfoiProperty, SeEnumequipenhanceeBuildType, SeEnumTownItemeTypeA, SeEnumequipattriColor, SeEnumequipsuperpowereType, SeEnumnoticetexteType } from "../Res/interface";
import { arrayRandomT,remove_duplicate } from "../TeTool";
import { TaskAction } from "../SeDefine";

declare var global: iApp;

export class SeEquipMgr {
    private equipenhanceColor = ['','akgreen','akblue','akpurple','akorange'];
    private enchantProperty = ['iUnitAtk','iUnitHp','iMagicAtk','iCastleAtk','iCastleHp'];
    private parent: SePlayer;
    constructor(p: SePlayer) {
        this.parent = p;
    }

    equip_opr(data, notice:boolean = true) {
        ...
        switch (data.type) {
            case 'add': {
                // 添加装备到装备库
                equip = new SeEquip(global.resMgr.getEquipRes(data.kId), this.parent.id, this.equipInfo.allEquips);
                this.equipInfo.allEquips++;
                this.equipInfo.haveEquips.push(equip);
                this.parent.taskAction(TaskAction.EquipAdd, 1, equip.eType, equip.iColor);
            ...
            case 'enhance': {
                ...
                if(equip.eLevel >= level_limit) return;

                let equipenhance = global.resMgr.getEquipenhanceRes(SeEnumequipenhanceeBuildType.QiangHua,equip.eType,equip.eLevel + 1);
                let items: [string] = equipenhance[this.equipenhanceColor[equip.iColor]];
                //使用道具
                for(let i = 0; i < items.length; i++){
                    let item = items[i];
                    //金币和其他item方式不一样
                    if(global.resMgr.TownItemRes.getRes(item.split(',')[0]).eTypeA == SeEnumTownItemeTypeA.JinYuanBao){
                    ...
                    if(i == items.length - 1){
                        equip.eLevel += 1;
                        //20级之后每10级需要通知
                        if(equip.iColor >= SeEnumequipattriColor.Cheng && equip.eLevel % 10 == 0 && equip.eLevel >= 20){
                            this.parent.sendAnnouncement(SeEnumnoticetexteType.ChengZhuangQiangHua, { equipname: equip.kId, equipLv: equip.eLevel, charname: this.parent.name}, this.parent);
                        }
               
                        this.parent.saveBaseInfo("equipInfo");
                    }
                }
                this.parent.taskAction(TaskAction.EquipEnhance, 1, equip.eType, equip.eLevel, equip.iColor);
            ...
            case 'addstar':{
                ...
                if(equip.eStar >= star_limit) return;
                let bfExp = equip.eEXP;
                var equip_res = global.resMgr.equipattrRes.getRes(equip.kId);
                let equipenhance = global.resMgr.getEquipenhanceRes(SeEnumequipenhanceeBuildType.ShengXing,equip.eType,equip.eStar + 1);
                let items: [string] = equipenhance[this.equipenhanceColor[equip.iColor]];
                //升星
                ...
                while(equip.eStar < equip_res.akstarExp.length - 1 && equip.eEXP >= parseInt(equip_res.akstarExp[equip.eStar].split(',')[1])){
                    if(equip.eStar >= star_limit) break;
                    equip.eEXP = equip.eEXP - parseInt(equip_res.akstarExp[equip.eStar].split(',')[1]);
                    equip.eStar++;
                    isAddStar = true;
                    this.parent.taskAction(TaskAction.EquipStar, 1, equip.eType,equip.eStar, equip.iColor);
                    global.logMgr.equipLog(this.parent, 'addstar', equip.eId, equip.kId, [], equip.eStar, equip.eStar - 1, equip.enchant);
                }
                //4星之后需要通知
                if(isAddStar && equip.iColor >= SeEnumequipattriColor.Cheng && equip.eStar >= 4){
                    this.parent.sendAnnouncement(SeEnumnoticetexteType.ChengZhuangShengXing, { equipname: equip.kId, equipStar: equip.eStar, charname: this.parent.name}, this.parent);
                }
                this.parent.taskAction(TaskAction.EquipExp, 1, equip.eType,exp_add_all);
            ...
            case 'decompose':{
                ...
                for(let k = 0; k < equipIds.length; k++){
                    ...
                    let equipenhance_star = global.resMgr.getEquipenhanceRes(SeEnumequipenhanceeBuildType.ShengXing,equip.eType,equip.eStar);
                    let items_star: [string] = equipenhance_star[this.equipenhanceColor[equip.iColor] + 'decompose'];
                    for(let i = 0; i < items_star.length; i++){
                        if(items_star[i].split(',')[0] == 'W002'){
                            remove_duplicate({kItemID: items_star[i].split(',')[0], iPileCount: Number(items_star[i].split(',')[1]) * (exp_all > 0?  exp_all - 1 : exp_all)}, item_all);
                        }
                        else{
                            remove_duplicate({kItemID: items_star[i].split(',')[0], iPileCount: Number(items_star[i].split(',')[1]) * exp_all}, item_all);
                        }
                    }
                    //强化费用也返还
                    let equipenhance_level = global.resMgr.getEquipenhanceRes(SeEnumequipenhanceeBuildType.QiangHua,equip.eType,equip.eLevel);
                    let items_level: [string] = equipenhance_level[this.equipenhanceColor[equip.iColor] + 'decompose'];
                    for(let i = 0; i < items_level.length; i++){
                        remove_duplicate({kItemID: items_level[i].split(',')[0], iPileCount: Number(items_level[i].split(',')[1])}, item_all);
                    }
                    //删除装备
            ...
            case 'enchant':{
                    ...
                    if(curr > 0){
                        ...
                        for(let k = 0; k < lock_items.length; k++){
                            if(lock_items[k].split(',')[0] == curr.toString()){
                                ...
                                remove_duplicate({kItemID: 'W110', iPileCount: Number(lock_items[k].split(',')[1])}, useItems);
                            }
                        }
                    }
                }
                ...
                remove_duplicate({kItemID: 'W002', iPileCount: gold_count}, useItems);
                //消耗附魔石
                ...
                remove_duplicate({kItemID: 'W109', iPileCount: enchant_count}, useItems);
                //创建池子
                let random_pool = [];
                ...
                for(let i = 0; i < this.enchantProperty.length; i++){
                    ...
                    let lucky = arrayRandomT(random_pool, "iPro");
                ...
                this.parent.taskAction(TaskAction.EquipEnchant, 1, equip.eType, equip.iColor);
            ...
            case 'chongzhu':{
                ...
                if(this.parent.baseInfo.next_chongzhu_time < Date.now()){
                    ...
                    if(!old_equip|| old_equip.iColor != SeEnumequipattriColor.Cheng || old_equip.isLocked || old_equip.isWeared) return;
    ...
    private check_all_zero(equip: SeEquip){
        for(let i = 0; i < this.enchantProperty.length; i++){
            let key = this.enchantProperty[i];
    ...
    public cheat_add_equip(kId, eLevel, eStar){
        let equip = new SeEquip(global.resMgr.getEquipRes(kId), this.parent.id, this.equipInfo.allEquips);
    ...
    public cheat_set_equip_All(eLevel, eStar){
     ...
     * 获取装备
     */
    public getHaveEquip(equipID: string): SeEquip {
        for (var i = 0; i < this.equipInfo.haveEquips.length; i++) {
    ...
    get equipInfo() {
     ...
     //获取装备固定和百分比加成总额
     public getEquipAddByType(equips?: Array<object>){
            ...
            for(let i = 0; i < wearEquips.length; i++){
                ...
                for(let j = 0 ; j < equipRes.akSuperpowerStar.length; j++){
                    if(equip.eStar >= parseInt(equipRes.akSuperpowerStar[j])){
                        ...
                        if(superpowerres.eType == SeEnumequipsuperpowereType.DanWeiGongJiTiShengBaiFenBi){
                            equipAdd[0] += superpowerres.iValue * 100;
                        }
                        else if(superpowerres.eType == SeEnumequipsuperpowereType.DanWeiGongJiTiShengGuDingZhi){
                            equipAdd[1] += superpowerres.iValue;
                        }
                        else if(superpowerres.eType == SeEnumequipsuperpowereType.DanWeiXueLiangTiShengBaiFenBi){
                            equipAdd[2] += superpowerres.iValue * 100;
                        }
                        else if(superpowerres.eType == SeEnumequipsuperpowereType.DanWeiXueLiangTiShengGuDingZhi){
    ...
    private useItems(items, count:number): boolean{
        for(let i = 0; i < items.length; i++){
            let item = items[i];
            if(global.resMgr.TownItemRes.getRes(item.split(',')[0]).eTypeA == SeEnumTownItemeTypeA.JinYuanBao){
    ...
    private delEquipsAndGetMaxLevel(use_equip_ids, now_level): number{
        //删除升星所需消耗装备
        for(let i = 0; i < use_equip_ids.length; i++){
