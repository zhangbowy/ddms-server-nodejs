...
import { arrayRandom, TeDate, TeMath, HashMap, arrayDel, TeMap, arrayRandomT, remove_duplicate, remove_duplicatev2 } from '../TeTool';
import { HeroBoxColorScore, SeShopInfo, SeMallItem, EnumMallType, SeMallFullItem, ActivityLog, ifShopLimit, DeAddDelItemReason } from './SePlayerDef';
import { PayTypeLog } from '../TeLogMgr';
import { TaskAction, func_copy } from '../SeDefine';
import { ifResHeroBoxPool, SeResHeroBox } from '../ResMgr/SeResMgr';
import { configInst } from '../lib/TeConfig';

declare var global: iApp;

export interface ifLevelTeHui {
    kID: string,
    level: number,
    starttime: number,
    finishtime: number,
...
export interface ifOpenBoxUnit {
    id: string,
    weight: number,
    type: number,
    orgweight: number,
    num: number,
    ipool: number,
    whithout_hand: boolean
}

export interface ifBMSale {
    /**
     * 卡牌id
     */
    kID: string,
...
export class SeShopMgr {
    private _parent: SePlayer;
    private _shopInfo: SeShopInfo;
    ...
    public initDb(d: any) {
        this._shopInfo = new SeShopInfo(d);
        this._checkDaily();
    ...
    private _item_sort(a: SeMallFullItem, b: SeMallFullItem) {
        if (a.iPos > b.iPos) return 1;
        if (a.iPos < b.iPos) return -1;
        return a.kID > b.kID ? 1 : -1;
    }

    public check_platform_limit(pkShopMallRes: TableRes.SeResShopMall) {
    ...
    public getInfo(): any {
        ...
        var d: Array<{ list: Array<SeMallFullItem>, updateTime?: number }> = [];
        var list: Array<Array<TableRes.SeResShopMall>> = [];
        ...
        for (var kid in res) {
            var pkShopMallRes: TableRes.SeResShopMall = res[kid];
        ...
        var item: SeMallFullItem;
        var resultList: SeMallFullItem[];
        for (var i = 0; i < list.length; i++) {
            if (!d[i]) continue;
            resultList = d[i].list;
            for (let j = 0; j < list[i].length; j++) {
                pkShopMallRes = list[i][j];
                if (this.check_platform_limit(pkShopMallRes)) continue;
    ...
    public payToReFresh(type: TableRes.SeEnumShopMalleType = TableRes.SeEnumShopMalleType.MeiRiJingXuan) {
    ...
    private _get_modfiy_item(type, pos?) {
        if (!this._shopInfo.modfiyItems) return null;
        if (!this._shopInfo.modfiyItems[type]) return null;
        if (pos == undefined || pos == -1 || pos == null) {
            return this._shopInfo.modfiyItems[type];
        }
        if (!this._shopInfo.modfiyItems[type][pos]) return null;
        return this._shopInfo.modfiyItems[type][pos];
    }

    private _set_modfiy_item(type, pos?, value?) {
        if (!this._shopInfo.modfiyItems) this._shopInfo.modfiyItems = [];
        if (!this._shopInfo.modfiyItems[type]) this._shopInfo.modfiyItems[type] = [];
        if (pos == undefined || pos == -1 || pos == null) {
            this._shopInfo.modfiyItems[type] = [];
            return;
        }
    ...
    //更新动态的礼包
    //目前就是轮换礼包
    public update_dynamic_res() {
        var result = {};
        var ids = global.resMgr.ShopMallRes.update();
        //筛选出需要同步的商品类型
        for (let i = 0; i < ids.length; i++) {
            let pkShopMallRes = global.resMgr.ShopMallRes.getRes(ids[i]);
            if (!result[pkShopMallRes.eType]) { result[pkShopMallRes.eType] = []; }
        }

        var res = global.resMgr.ShopMallRes.getAllRes();
        for (var kid in res) {
            let _pkShopMallRes: TableRes.SeResShopMall = res[kid];
            let item = this._getFullItem(_pkShopMallRes);
            if (result[_pkShopMallRes.eType]) result[_pkShopMallRes.eType].push(item);
    ...
    //现在更新玩家的轮换礼包限制次数
    //每次登录的时候 判断一下
    public update_runhuan_gift() {
        for (var kid in this._shopInfo.limit_count) {
            let _pkShopMallRes: TableRes.SeResShopMall = global.resMgr.ShopMallRes.getRes(kid);
     ...
     * @param lastHours 
     */
    public update_shop_limit_time(mallid: string, lastHours: number) {
        let pkMallRes = global.resMgr.ShopMallRes.getRes(mallid);
        if (!pkMallRes) return;
        if (this.check_platform_limit(pkMallRes)) return;
    ...
    private get_shop_limit(mallId: string) {
        var pkShopMallRes = global.resMgr.ShopMallRes.getRes(mallId);
        if (!pkShopMallRes)
            return {
                buy: 0,
        ...
        var limit_info: ifShopLimit = this._shopInfo.limit_count[mallId];
                ...
                if (typeof limit_info == 'number') {
                    ...
                    switch (pkShopMallRes.eFreshType) {
                        ...
                        case TableRes.SeEnumShopMalleFreshType.MeiRi:
                            if (TeDate.isdiffday(limit_info.cttime)) {
                                bfresh = true;
                            }
                            break;
                        case TableRes.SeEnumShopMalleFreshType.MeiZhou:
                            if (TeDate.isdiffweek(limit_info.cttime)) {
                                bfresh = true;
                            }
                            break;
                        case TableRes.SeEnumShopMalleFreshType.MeiYue:
                            if (TeDate.isdiffmonth(limit_info.cttime)) {
                                bfresh = true;
                            }
                            break;
                        ...
                        case TableRes.SeEnumShopMalleFreshType.SiBaXiaoShi: {
                            let time = configInst.get("cheatmode") ? 60 * 1000 : 48 * 60 * 60 * 1000;
                        ...
                        case TableRes.SeEnumShopMalleFreshType.ShiErXiaoShi: {
                            let time = configInst.get("cheatmode") ? 60 * 1000 : 12 * 60 * 60 * 1000;
                            if (Date.now() - limit_info.cttime >= time) {
                                bfresh = true;
                            }
                            break;
                        ...
                        case TableRes.SeEnumShopMalleFreshType.ErSiXiaoShi: {
                            let time = configInst.get("cheatmode") ? 60 * 1000 : 24 * 60 * 60 * 1000;
                            if (Date.now() - limit_info.cttime >= time) {
                                bfresh = true;
                            }
                            break;
                        ...
                        case TableRes.SeEnumShopMalleFreshType.MeiLiangRi: {
                            if (TeDate.daydiff(limit_info.cttime) >= 2) {
                                bfresh = true;
                            }
                            break;
                        }
    ...
    // 限购判断
    public is_shop_limit(mallId: string, count: number) {
        let pkShopMallRes = global.resMgr.ShopMallRes.getRes(mallId);
        let limit_info: ifShopLimit = this.get_shop_limit(mallId);
    ...
    public add_shop_limit(mallId: string, count: number = 1) {
        var pkShopMallRes = global.resMgr.ShopMallRes.getRes(mallId);
        //限时礼包不走这里
        ...
        var limit_info: ifShopLimit = this.get_shop_limit(mallId);
        if (pkShopMallRes && pkShopMallRes.iCount > 0) {
            limit_info.buy += count;
            this._shopInfo.limit_count[mallId] = limit_info;
            this.saveInfo("limit_count");
            if (this._parent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.Player.linkid, 'limit_count', mallId, this._shopInfo.limit_count[mallId]);
        }
    }

    public buyShopMall(mallId: string, marketId: string, count: number = 1) {
        var pkShopMallRes = global.resMgr.ShopMallRes.getRes(mallId);
        if (!pkShopMallRes) return false;

        if (pkShopMallRes.eType != TableRes.SeEnumShopMalleType.JinJiTeHui) {
            if (pkShopMallRes.iOpenRank && pkShopMallRes.iOpenRank > this.Player.pvp_level) return false;
            if (pkShopMallRes.iOpenELO && pkShopMallRes.iOpenELO > this.Player.pvp_score) return false;
            if (pkShopMallRes.iOpenJJCRank && pkShopMallRes.iOpenJJCRank > this.Player.max_pve_pk_rank) return false;
        }
        ...
        var priceType: number = TableRes.SeEnumSuperMarketeType.JinBi;
        var mallType: EnumMallType = EnumMallType.Item;

        // 强制关闭时间
        // if (pkShopMallRes.kForceFinishTime && (new Date(pkShopMallRes.kForceFinishTime)).getTime() < Date.now()) return false;

        var mallInfo: SeMallItem;
        var pkMarketRes: TableRes.SeResSuperMarket = global.resMgr.SuperMarketRes.getRes(marketId);
        var iDiscount = 1;
        if ([TableRes.SeEnumShopMalleType.MeiRiJingXuan, TableRes.SeEnumShopMalleType.CangBaoGe].indexOf(pkShopMallRes.eType) >= 0) {
            mallInfo = this._get_modfiy_item(pkShopMallRes.eType, pkShopMallRes.iPos) as SeMallItem;
            if (!mallInfo) return false;

            if (mallInfo.mallType == EnumMallType.Item) {
                pkMarketRes = global.resMgr.SuperMarketRes.getRes(marketId);
                if (!pkMarketRes) return false;
                if (!isNaN(pkMarketRes.iDiscount)) iDiscount = pkMarketRes.iDiscount;
        ...
        if (pkShopMallRes.eType == TableRes.SeEnumShopMalleType.WanBaTeJia) {
            if (configInst.get('plt') != 'qzone' || (this._parent.ext_vip_level < (pkShopMallRes.aiValue[0] || 0)))
                return false;
        }
        ...
        if(pkShopMallRes.eType == TableRes.SeEnumShopMalleType.ShenMiShangDianYingXiong){
            if(this.Player.baseInfo.selectHeros.indexOf(pkShopMallRes.kExtraValue) == -1) return false;
        ...
        if(pkMarketRes && pkMarketRes.akExtraPay){
            for(let i = 0; i < pkMarketRes.akExtraPay.length;i++){
                if (!this.Player.delItem(pkMarketRes.akExtraPay[i].split(',')[0], Number(pkMarketRes.akExtraPay[i].split(',')[1]), DeAddDelItemReason.shopdel, mallId)) return false;
            }
        }
        ...
        switch (priceType) {
            case TableRes.SeEnumSuperMarketeType.JinBi:
                if (price && !this._parent.useGold(price)) return false;
                payType = PayTypeLog.jinbi;
                break;
            case TableRes.SeEnumSuperMarketeType.ZuanShi:
                if (price && !this.Player.decMoney(price, 'buyitem', marketId, num.toString())) return false;
                payType = PayTypeLog.zuanshi;
                break;
            case TableRes.SeEnumSuperMarketeType.DaoJu:
                if (price && !this.Player.delItem(payId, price, DeAddDelItemReason.shopdel, mallId)) return false;
                payType = payId;
                break;
            case TableRes.SeEnumSuperMarketeType.MeiRiCiShu:
                if (price) {
                    if (!this.Player.dailyInfo.hasOwnProperty(payId)) return false;
                    if (this.Player.dailyInfo[payId] < price) return false;
        ...
        if (mallInfo && mallType == EnumMallType.Hero) {
        ...
        else if(mallInfo && mallType == EnumMallType.Equip) {
            for (var i = 0; i < mallInfo.akItemID.length; i++) {
                var item = mallInfo.akItemID[i].split(',');
                this.Player.addItem(item[0], (parseInt(item[1]) || 1) * num, DeAddDelItemReason.shopbuy, mallId);
            }
        ...
        // 触发购买事件
        this._parent.taskAction(TaskAction.ShopBuy, mallId, num);
    }
     ...
     * @param chanceId 
     */
    get_random_pool(boxid: string) {
        let outs = [];
        let res_herobox = global.resMgr.HeroBoxZZYRes.getRes(boxid);
        if (!res_herobox) {
            res_herobox = global.resMgr.getResHeroBoxZZY(0, 0);
            if (!res_herobox) return global.netMgr.sendData({ cmd: 'randompool', boxid: boxid, pools: outs })
        }
    ...
    private _getFullItem(res: TableRes.SeResShopMall): SeMallFullItem {
        let curr = Date.now()

        var result: SeMallFullItem = {
            kID: res.kID,
        ...
        if (idx >= 0) {
            let target = this._get_modfiy_item(res.eType, res.iPos) as SeMallItem;
            if (target) {
                for (let key in target) {
                    if (key == 'iCount') continue;
                    result[key] = target[key];
                }
                if (target.mallType == EnumMallType.Item) {
                    let pkSuperMarketRes: TableRes.SeResSuperMarket = global.resMgr.SuperMarketRes.getRes(target.marketId);
                    if (pkSuperMarketRes) {
            ...
            result.mallType = (target.eItemType == TableRes.SeEnumSuperMarketeItemType.Item) ? EnumMallType.Item : EnumMallType.Hero;
    ...
    protected _checkDaily(force = false) {
    ...
    private _limit_clear(type: TableRes.SeEnumShopMalleType) {
        let shop_list: SeMallItem[] = this._get_modfiy_item(type) as SeMallItem[];
        if (shop_list) {
            for (var i = 0; i < shop_list.length; i++) {
                let sssr = shop_list[i];
                if (sssr) {
                    let res = global.resMgr.ShopMallRes.getRes(sssr.kID);
                    if(res && res.eFreshType == TableRes.SeEnumShopMalleFreshType.BuShuaXin) continue;
    ...
    protected _random_shop_mall_item(Mallid: string, excepts: string[]): { type: TableRes.SeEnumShopMalleItemType, value: { item: string, count: number, price: number } | string } {
        let out: { type: TableRes.SeEnumShopMalleItemType, value: { item: string, count: number, price: number, type?: number } | string };
        let res = global.resMgr.ShopMallRes.getRes(Mallid);
        if (!res) return out;

        let parsed_conntents: { v: string, p: number }[] = [];
        for (let i = 0; i < res.akContent.length; i++) {
            ...
            if (excepts.indexOf(p_one.v) >= 0) continue;
        ...
        switch (res.eItemType) {
            ...
            case TableRes.SeEnumShopMalleItemType.Chance: {
                ...
                if (res.akContent.length > 1) {
                    // 需要随机
                    luck = arrayRandomT(parsed_conntents, 'p')
                }
                ...
                for (let i = 0; i < pool.length; i++) {
                    let r_unit = pool[i];
                    if (excepts.indexOf(r_unit.id) >= 0) continue;
                    s_pool.push(r_unit);
                }

                let luckItem = arrayRandomT(s_pool, 'weight');
                if (!luckItem) break;
            ...
            case TableRes.SeEnumShopMalleItemType.Supermarket:
                ...
                if (parsed_conntents.length > 1) {
                    luck = arrayRandomT(parsed_conntents, 'p').v;
                }
    ...
    private _modfiy_item_change(type: number, iPos: number, res, luckItem) {
        if (luckItem.type == TableRes.SeEnumShopMalleItemType.Unit || luckItem.type == TableRes.SeEnumShopMalleItemType.Chance) {
            // market
            let selItem = luckItem.value as { item: string, count: number, price: number, type: number };
            ...
            this._set_modfiy_item(type, iPos, {
                ...
                mallType: selItem.type == undefined? EnumMallType.Hero : selItem.type,
            ...
            let selItem = luckItem.value as string;
            this._set_modfiy_item(type, iPos, {
                kID: res.kID,
                marketId: selItem,
                mallType: EnumMallType.Item,
                iCount: res.iCount,
            });
        }
    ...
    protected _refreshCangBaoGeShop() {
        // 这里一共有6个格子，给玩家随机一下实际销售的道具
        let mallResHash: Array<string> = [];
        let maxPos = 0;

        let open_types: TableRes.SeResShopMall[] = [];
        // 先判断一下开放级别等信息
        let types = global.resMgr.getShopMailByType(TableRes.SeEnumShopMalleType.CangBaoGe);
        for (let i = 0; i < types.length; i++) {
            let pkRes = types[i];
            if (!pkRes || this.Player.top_pvp_level < pkRes.iOpenGrade) continue;
    ...
    private _isShopResOpen(pkShopRes: TableRes.SeResSuperMarket) {
        var succ = false;
        if (!pkShopRes) return succ;
        switch (pkShopRes.eOpen) {
            case TableRes.SeEnumSuperMarketeOpen.GuanBi: succ = false; break;
            case TableRes.SeEnumSuperMarketeOpen.KaiQi: succ = true; break;
            default: break;
        }
    ...
    public queryShopItem(kItemID: string) {
        var out = [];
        if (kItemID == 'all') {
    ...
    public openboxnotice(boxID, cardList: Array<{ kid: string, num: number }>) {
    ...
    private checkAndAddLuckyBoxRecord(itemid, itemNum, obj){
    ...
    private _openCardColor(cardList: Array<{ kid: string, num: number }>) {
        ...
        this._parent.taskAction(TaskAction.OpenCard, colorList);
    }
    ...
    public openBox(itmeid: string, level: number, type: number = 1) {
        ...
        if (this._parent.bInitComplete) global.netMgr.sendCharMiscUpdate(this.Player.linkid, 'ocard', { cards: cardList, changes: out['changes'], type: type, level: level, boxid: itmeid, count: 1 });
        this._parent.taskAction(TaskAction.OpenBox, level, type, 1);
        this._openCardColor(cardList);
        this.openboxnotice(res_herobox.kID, cardList);
    }

    private _openBoxBatch_(itemid: string, count: number, boxId: string, cheat: boolean = false, isBox: boolean = true, notice: boolean = true) {
        ...
        if (cheat) {
            ...
            if (this._parent.bInitComplete && notice) global.netMgr.sendCharMiscUpdate(this.Player.linkid, 'ocard', { cards: cardList, changes: out['changes'], type: res_herobox.eType, level: res_herobox.iLevel, boxid: itemid, count: count });
            if (isBox) this._parent.taskAction(TaskAction.OpenBox, res_herobox.iLevel, res_herobox.eType, count);
            this._openCardColor(cardList);
        }
    ...
    public openBoxBatch(itmeid: string, count: number, level: number, type: number, cheat: boolean = false, isBox: boolean = true, notice: boolean = true) {
        count = Math.max(Math.min(count, 1000), 1);
        var res_herobox = global.resMgr.getResHeroBoxZZY(level, type);
        if (res_herobox) return this._openBoxBatch_(itmeid, count, res_herobox.kID, cheat, isBox, notice);
    }

    private _randomCard(cards: Array<string>, exp: Array<string>) {
        var totalCard = this.Player.totalCard;
        var totalScore = this.Player.totalCardScore;
        var totalHero = this.Player.totalHero;

        var cardList: Array<{ kid: string, num: number }> = [];

        var totalP = 0;
        for (var i = 0; i < cards.length; i++) {
            var heroID = cards[i];
            if (exp.indexOf(heroID) >= 0) continue;
            ...
            if (!rkCard) {
                ...
                tA = (global.resMgr.cardSumCostToLevel(rkCard.iLevel - 1) + rkCard.iCount + 1) * (HeroBoxColorScore[pkHeroCardRes.iColour] || 1);
            }
    ...
    //  * @param kGiftID 礼包的id
    //  */
    openChanceBox(itmeid: string, kGiftID: string, num: number, notice: boolean = true) {
    ...
    public test_rand_cards(boxID: string, count: number) {
    ...
    private randomUpTown(num: number) {
        // 获取小数部分
    ...
    private poolName2use = {};

    getPoolNameUse(poolname: string) {
        return this.poolName2use[poolname] || 0;
    }

    addPoolNameUse(poolname: string) {
     ...
     */
    private _rand_card_mod_2_init(res_herobox: SeResHeroBox, name_pools: { weight: number, pool: Array<ifOpenBoxUnit>, s_pool: Array<ifOpenBoxUnit>, pool_name: string }[]) {
        let random_pool: ifResHeroBoxPool[] = [];
        ...
        if (!luck_boy) {
            luck_boy = arrayRandomT(random_pool, "iweight");
        }
        ...
        // 从池子里面取出一个
        let sel_pool: { weight: number, pool: Array<ifOpenBoxUnit>, s_pool: ifOpenBoxUnit[], pool_name: string } = null;
        ...
        if (sel_pool && sel_pool.pool.length > 0) {
            let remove = true;
            if(res_herobox.iReturnBack == 1) remove = false;
            let luck = arrayRandomT(sel_pool.pool, 'weight', remove);
    ...
    private _rand_pool_mod_2_ex(res_herobox: SeResHeroBox, random_pool: ifResHeroBoxPool[], select_boy?: ifResHeroBoxPool) {
        let n_key;
        ...
        if (Object.keys(n_map).length == 0) {
            for (let i in res_herobox.akCardPools) {
                ...
                n_map[_herobox.name] = (n_map[_herobox.name] || 0) + 1 / _rate + TeMath.get_gaussian_distribution_table_reverse(Math.random()) / _rate / 3;
            }
        }
        ...
        n_map[luck_boy.name] = (n_map[luck_boy.name] || 0) + 1 / __rate + TeMath.get_gaussian_distribution_table_reverse(Math.random()) / __rate / 3;
     ...
     * 抽卡流程 2017年10月14日 动工
     */
    public _rand_cards_(chanceId: string, rand_level = this.Player.top_pvp_level) {
        ...
        let outs: Array<{ kid: string, type: number, num: number, extnum: number }> = [];
        ...
        let luck_cards: ifOpenBoxUnit[] = [];
        for (let i = 0; i < res_herobox.akCardPools.length; i++) {
            let rPool = res_herobox.akCardPools[i];
            let color_count = rPool.imin;
            if (!color_count) continue;
            if (rPool.imaxuse > 0 && rPool.imaxuse <= this.getPoolNameUse(rPool.name)) continue;
        ...
        let out_heros: HashMap<{ id: string, count: number, type: number, num: number, orgweight: number, whithout_hand: boolean }> = new HashMap<{ id: string, count: number, type: number, num: number, orgweight: number, whithout_hand: boolean }>();
        ...
        for (let i = 0; i < luck_cards_weight.length; i++) {
            ...
            if (i != luck_cards_weight.length - 1) {
                count = r_n * r_hero.weight + Math.sqrt(r_n) * r_hero.weight * TeMath.get_gaussian_distribution_table_reverse(Math.random());
            }
        ...
        for (let i = 0; i < res_herobox.akCardPools.length; i++) {
            ...
            let rPool = res_herobox.akCardPools[i];
            if (!i_r_heros || i_r_heros.length == 0 || !rPool) continue;
        ...
        let realOuts: Array<{ kid: string, type: number, num: number }> = [];
     ...
     * @return 随机的金额
     */
    private _rand_gold(base_gold_min: number, base_gold_max: number): number {
        if (base_gold_max == 0) return 0;
     ...
     * @param mxColor 
     */
    private _rand_card_mod_1_init(ilevel: number, akCardPools: ifResHeroBoxPool[], akEquitablePool: string[]) {
        var name_pools: { weight: number, pool: Array<ifOpenBoxUnit>, s_pool: ifOpenBoxUnit[], pool_name: string }[] = [];
     ...
     * @param ilevel 
     */
    private _rand_card_color_init(ipool: number, ilevel: number, poolcof: ifResHeroBoxPool, akEquitablePool: string[]): { weight: number, pool: Array<ifOpenBoxUnit>, s_pool: ifOpenBoxUnit[], pool_name: string } {
        let pool_name: string = poolcof.name;
        ...
        if (!color_pool || color_pool.length == 0) return { weight: 0, pool: [], s_pool: [], pool_name: pool_name };

        var total_weight: number = 0;
        var hero_weights: Array<ifOpenBoxUnit> = [];
     ...
     * 需要考虑每个颜色的权重
     */
    private _random_luck_pool_unit(res: TableRes.SeResHeroBoxZZY, rPool: ifResHeroBoxPool, name_pools: { weight: number, pool: Array<ifOpenBoxUnit>, pool_name: string }[]): {
        total_weight: number,
        pool: Array<ifOpenBoxUnit>
    } {
        var out: { total_weight: number, pool: Array<ifOpenBoxUnit> } = {
            total_weight: 0,
            pool: []
        };;
     ...
     * @param hero_weights 
     */
    private _rand_card_result(total_weight: number, hero_weights: Array<ifOpenBoxUnit>): { luck: ifOpenBoxUnit, total_weight: number, pool: Array<ifOpenBoxUnit> } {
        var luck_boy = total_weight * Math.random();
        var luck_hero: ifOpenBoxUnit;
    ...
    private _get_hero_base_weight(heroid: string, extRate: number, random_card: boolean) {
        var tA = 1;
        if (!random_card) {
    ...
    private _cards_weight_sort(a: ifOpenBoxUnit, b: ifOpenBoxUnit) {
        if (a.ipool < b.ipool) {
            return 1;
        }
     ...
     * 初始化卡组数据
     */
    private _rand_cards_count_init(ilevel: number, res: TableRes.SeResHeroBoxZZY, lucky_boys: ifOpenBoxUnit[], akCardPools: ifResHeroBoxPool[]) {
        /**
         * 所有武将品质权重和
         */
        ...
        var color_count: Array<number> = [];
        var color_weights: Array<number> = [];

        // 初始化基础权重和收集颜色
        for (var i = 0; i < lucky_boys.length; i++) {
            var rLuckyHero = lucky_boys[i];
            if (!rLuckyHero) continue;
     ...
     * @param ilevel 
     */
    private _rand_card_init(ipool: number, ilevel: number, poolname: string, weight: number, shangjinUsed: boolean): { weight: number, pool: Array<ifOpenBoxUnit>, s_pool: ifOpenBoxUnit[], pool_name: string } {
        var color_pool = global.resMgr.getChanceList(poolname, ilevel, true);
        if (!color_pool || color_pool.length == 0) return { weight: 0, pool: [], s_pool: [], pool_name: poolname };
    ...
    private shangjinPoolFlush(chance: any) {
        ...
        if (!this._parent.baseInfo.shangjinHeroPool[chanceId]) this._parent.baseInfo.shangjinHeroPool[chanceId] = {
            weight: 0, 
            pool: [], 
            s_pool: []
        };
    ...
    public activityFlush(activityId: string) {
    ...
    public poolFlush(chanceId: string) {
        let chanceRes: TableRes.SeResHeroBoxEgg = global.resMgr.HeroBoxEggRes.getRes(chanceId);
        ...
        for (var i = 0; i < chanceRes.akPoolName.length; i++) {
            ...
            let luck = arrayRandomT(sel_pool.pool, 'weight', true);
            if(luck){
        ...
        if (!this._parent.baseInfo.chancePools[chanceId]) this._parent.baseInfo.chancePools[chanceId] = {};
    ...
    public poolRandom(chanceId: string): { tid: string, num: number } {
        ...
        //copy
        let retPool: Array<Object> = func_copy(curPool);

        let item;
        while (retPool.length > 0) {
            item = arrayRandomT(retPool, 'weight', true);
            if (item.limit <= count) break;
        }
    ...
    // 检查升级特惠信息
    checkLevelTehui(level: number = 0) {
        var hasNoBuy = false;
        var nTime = Date.now();
        var pvpInfos = this.dbInfo.get('leveltehui') as Array<ifLevelTeHui>;
        if (pvpInfos) {
            var lft = [];
    ...
    finishLevelTehui(Mailid: string) {
        var bfind = false;
        var pvpInfos = (this.dbInfo.get('leveltehui') || []) as Array<ifLevelTeHui>;
    ...
    addLevelTehui(level: number) {
        ...
        var s_list = list[TableRes.SeEnumShopMalleType.JinJiTeHui] as Array<TableRes.SeResShopMall>;
        if (s_list) {
            var level_res: TableRes.SeResShopMall;
            ...
            if (level_res) {
                var leveltehui = (this.dbInfo.get('leveltehui') || []) as Array<ifLevelTeHui>;
                ...
                if (level_res) this.updateTeHuiInfo2Player();
            }
        }
    }

    public updateTeHuiInfo2Player() {
    ...
    public exchange(activityId: string, exchangeId: string, extInfo: string, excount: number) {
        ...
        if (!pkExchangeRes) return;
        var activityLog: ActivityLog = { kActivityId: activityId, kExchangeId: exchangeId, buyCount: 0, extInfo: extInfo };
        if (activityId != 'exchange') {
            // 不判断活动的就不需要检查开启时间和数量了
            var pkActivityRes = global.resMgr.activityRes.getRes(activityId);
            if (!pkActivityRes) return;
            if (pkActivityRes.kStartTime && Date.parse(pkActivityRes.kStartTime) > Date.now()) return;
            if (pkActivityRes.kEndTime && Date.parse(pkActivityRes.kEndTime) < Date.now()) return;
            ...
            for (var i = 0; i < pkActivityRes.akExChanges.length; i++) {
                ...
                if (eId == exchangeId) {
                    if (limitCount >= 0) remindCount = limitCount - activityLog.buyCount;
                    break;
                }
            }
            if (remindCount < excount) return;
        }
        ...
        switch (pkExchangeRes.ePriceType) {
            case TableRes.SeEnumExchangeePriceType.JinBi:
                if (pkExchangeRes.iConsumable == 0) {
                    // 表示不消耗的
                    if (this._parent.gold < price) return false;
                }
                else {
                    if (!this._parent.useGold(price)) return false;
                }
                break;
            case TableRes.SeEnumExchangeePriceType.ZuanShi:
                if (pkExchangeRes.iConsumable == 0) {
                    // 表示不消耗的
                    if (this._parent.money < price) return false;
                }
                else {
                    if (!this.Player.decMoney(price, 'exchange', exchangeId, targets.toString())) return false;
                }
                break;
            case TableRes.SeEnumExchangeePriceType.DaoJu:
                var itemNum = this._parent.itemCount(pkExchangeRes.kPriceItem);
                if (itemNum < price) return false;
                if (pkExchangeRes.iConsumable == 1) this._parent.delItem(pkExchangeRes.kPriceItem, price, DeAddDelItemReason.exchangecost, exchangeId);
                break;
            case TableRes.SeEnumExchangeePriceType.DuoZhongDaoJu:
                ...
                for(let i = 0; i < items.length; i++){
                    ...
                    if(itemId == 'W001'){
                        if (this._parent.money < itemPrice) return false;
                        if (pkExchangeRes.iConsumable == 1 && !this.Player.decMoney(itemPrice, 'exchange', exchangeId, targets.toString())) return false;
                    }
                    else if(itemId == 'W002'){
                        if (this._parent.gold < itemPrice) return false;
                        if (pkExchangeRes.iConsumable == 1 && !this._parent.useGold(itemPrice)) return false;
                    }
                    else{
                        if (this._parent.itemCount(itemId) < itemPrice) return false;
                        if (pkExchangeRes.iConsumable == 1) this._parent.delItem(itemId, itemPrice, DeAddDelItemReason.exchangecost, exchangeId);
                    }
                }
                break;
            default: return false;
        ...
        if (pkExchangeRes.eType == TableRes.SeEnumExchangeeType.DaoJu) {
            for (let i in targets) {
                ...
                this._parent.addItem(args[0], parseInt(args[1]) * excount, DeAddDelItemReason.exchange, exchangeId);
            }
    ...
    private __exchange(activityId: string, exchangeId: string, excount: number) {
        //砸金蛋活动的刷新
        if (exchangeId == 'E055') {
            this._shopInfo.activityHash[activityId] = {};
        }
    ...
    private _box_view_(boxid: string) {
        var pkHeroBoxRes = global.resMgr.HeroBoxZZYRes.getRes(boxid);
        if (!pkHeroBoxRes) return [];

        var outItems: { id: string, type: number, num: number }[] = [];
        ...
        // 这里取出垃圾信息，只保留有效信息
        var shotInfoMap = new TeMap<{ id: string, type: number, num: number }>();
        for (var i = 0; i < outItems.length; i++) {
            var one_ = outItems[i];
            if (one_) shotInfoMap.set(one_.id + one_.num + one_.type, one_);
        }

        var out: { id: string, type: number, num: number }[] = [];
    ...
    reset_shop_limit(mallid: string) {
    ...
    public refreshLimitCount(mallid: string){
