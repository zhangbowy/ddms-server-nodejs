...
import { SeResLootPool, SeEnumChartTableeType, SeEnumLevelInfoiRankType, SeEnumLevelInfoiProperty, SeEnumTownItemeTypeA, SeEnumequipsuperpowereType, SeEnumActivityeType } from "../Res/interface";
import { TeDate, countBits, remove_duplicate } from "../TeTool";
import { configInst } from "../lib/TeConfig";
import { TaskAction, CharState, SeLogicFormation } from "../SeDefine";
import { stringify } from "querystring";

declare var global: iApp;

export class SePveNewMgr {
    private parent: SePlayer;
    private interval;
    constructor(p: SePlayer) {
        this.parent = p;
    }
    private get _baseInfo() {
    ...
    public addPower(count: number, isLimit: boolean){
    ...
    public dailyRefresh(){
        ...
        if(TeDate.isdiffday(this._baseInfo.free_fight_fresh_time)) {
    ...
    private _can_fight(levelId: string, isSweep: boolean) {
        if(global.resMgr.pveInfoRes.getRes(levelId).kbefore){
            var before = global.resMgr.pveInfoRes.getRes(levelId).kbefore.split(',');
            for(let i =0; i < before.length; i++){
                if(countBits(this.levelInfo[before[i]].star) == 0){
                    return false;
                }
            }
        }
        if(!this.levelInfo[levelId]) return false;
        if(this.levelInfo[levelId].freeTimes <= 0)  return false;
        if(this.parent.itemCount('W112') < Number(global.resMgr.pveInfoRes.getRes(levelId).kPriceID))  return false;
        if(isSweep && countBits(this.levelInfo[levelId].star) < 3) return false;
        return true;
    }

    startfight(levelId: string) {
        // 玩家开始某个地图的战斗
        // 检查是否能够开始
        if (!this._can_fight(levelId, false)) {
            ...
            this.parent.taskAction(TaskAction.PveFight, 1, levelId);
            this.parent.setState(CharState.inrace);
        }
        
    }

    private notice_to_player_start(succ: boolean, levelId: string) {
    ...
    test_finishfight(levelId, count){
        ...
        for(let i = 0; i < count; i++){
            let pkInfoRes = SeResMgr.inst.pveInfoRes.getRes(levelId);
            ...
            if (awards.length > 0){
                ...
                for(let i = 0; i < awards.length; i++){
                    ...
                    if(res.eTypeA == SeEnumTownItemeTypeA.DuanWeiBaoXiang){
                        ...
                        for(var key in out.ids){
                            if(out.ids[key] > 0){
                                remove_duplicate({kItemID: key, iPileCount:out.ids[key]}, outs);
                            }
                        }
                        for(let i = 0; i< out.changes.length; i++){
                            if(out.changes[i].cGolds > 0){
                                remove_duplicate({kItemID: 'W002', iPileCount:out.changes[i].cGolds}, outs);
                            }
                            if(out.changes[i].cItems > 0){
                                remove_duplicate({kItemID: 'BD01', iPileCount:out.changes[i].cItems}, outs);
                            }
                        }
                    }
                    else if(res.eTypeA == SeEnumTownItemeTypeA.LiBao){
                        ...
                        remove_duplicate(awards[i],outs);
                    }
                }
            } 
        }
    ...
    finishfight(levelId: string, hps: number[], time: number, isSweep: boolean) {
        ...
        if (awards.length > 0){
            ...
            for(let i = 0; i < awards.length; i++){
                ...
                if(res.eTypeA == SeEnumTownItemeTypeA.DuanWeiBaoXiang){
                    ...
                    remove_duplicate(awards[i],outs);
                    remove_duplicate(awards[i],needAdd);
                }
            }
        ...
        let race_type = 4444;
        if(isSweep){
            this.parent.taskAction(TaskAction.PveSweep, 1, levelId);
        }
        else{
            if(win){
                this.parent.taskAction(TaskAction.PveWin, 1, levelId);
            }
            //第一次3星不竞速
                        ...
                        if(levelId == levelIdRes && (this._baseInfo.level_speed_level == "" || this._baseInfo.level_speed_level == levelId)){
                            ...
                            if(!this._baseInfo.level_speed[i] || speed_time < this._baseInfo.level_speed[i]){
                                ...
                                if(chart_res.iCrossSever){
                                    global.globalChartMgr.addPlayerLevelChart(this.parent.pvpMgr.seasonid,
                                        pkRes.akLevelSpeed[i].split(',')[0],
                                        {
                                            id: this.parent.id,
         ...
         this.savebaseinfo('levelInfo');
         if(star_add > 0){
            this.parent.taskAction(TaskAction.PveStar, pkInfoRes.iType, countBits(star_add));
            //获取所有星数
            ...
            global.chartMgr.addPlayerLevelChart(this.parent.pvpMgr.seasonid,
                SeEnumChartTableeType.pveXingShuBang,
                {
                    id: this.parent.id,
                    name: this.parent.name,
                    score: score,
        ...
        this.parent.setState(CharState.loadcomplete);
        //更新限制武将牌状态
        this.parent.updateTryHeroCard('match');
    }

    public getFormationCheatScore(formation?: Array<SeLogicFormation>, equips?){
    ...
    public getAllStarCount(){
        let count = 0;
        for(var key in this.levelInfo){
            count += countBits(this.levelInfo[key].star);
        }
        return count;
    }

    private notice_to_player_finish(succ: boolean, win: boolean, star: number, freeTimes: number, levelId: string, awards: SeItem[] = []) {
        global.netMgr.sendData({
            cmd: 'pveNewaward',
            succ: succ,
            win: win,
    ...
    //添加战斗次数
    public addFreeTimes(levelId: string){
    ...
    //添加战斗可购买次数
    public addCanBuyTimes(count: number){
        for(var key in this.levelInfo){
